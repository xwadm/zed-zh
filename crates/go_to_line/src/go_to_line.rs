...
impl Render for GoToLine {
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let help_text = if let Some(offset) = self.relative_line_from_query(cx) {
            ...
            format!("跳转到行 {target_line}（相对于当前行 {offset:+}）").into()
        } else {
            match self.line_and_char_from_query(cx) {
                Some((line, Some(character))) => {
                format!("跳转到行 {line}，字符位置 {character}").into()
                }
                Some((line, None)) => format!("跳转到行 {line}").into(),
...
mod tests {
        ...
        workspace.update(cx, |workspace, cx| {
            assert_eq!(
                ...
                workspace
                    ...
                    .read(cx)
                    .selection_stats(),
                "选中包含多字节Unicode字符的文本后，字符计数应保持正确"
            );
        });
    }
    ...
    async fn test_go_into_unicode(cx: &mut TestAppContext) {
        ...
        for (i, c) in text.chars().enumerate() {
            ...
            assert_eq!(
                point,
                current_position(&workspace, cx),
                "跳转到位置 {point:?} 时，预期光标位于字符串 '{text}' 的字符 '{c}' 处",
            );
        }
    ...
    fn assert_single_caret_at_row(
        ...
        assert!(
            selections.len() == 1,
            "预期一个光标选区，实际得到：{selections:?}"
        );
        let selection = &selections[0];
        assert!(
            selection.start == selection.end,
            "预期单个光标选区，实际得到：{selection:?}"
        );
