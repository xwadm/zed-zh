                    ...
                    if let Some(worktree) = project.read(cx).worktree_for_entry(entry_id, cx)
                        && let Some(entry) = worktree.read(cx).entry_for_id(entry_id) {
                            ...
                            workspace
                                ...
                                .detach_and_prompt_err("无法打开文件", window, cx, move |e, _, _| {
                                    match e.error_code() {
                                        ErrorCode::Disconnected => if is_via_ssh {
                                            Some("与 SSH 主机断开连接".to_string())
                                        } else {
                                        ...
                                        ErrorCode::Internal if e.to_string().contains("文件过大，无法加载") => Some(e.to_string()),
                                        _ => None,
                                    }
                                });
                ...
                menu.context(self.focus_handle.clone()).map(|menu| {
                    if is_read_only {
                        ...
                        menu.action("新建文件", Box::new(NewFile))
                                ...
                                .when(is_local, |menu| {
                                    menu.action("使用默认应用打开", Box::new(OpenWithSystem))
                                })
                                ...
                                .when(has_git_repo, |menu| {
                                    menu.separator()
                                        ...
                                        .when(has_file_history, |menu| {
                                            menu.action("查看文件历史", Box::new(git::FileHistory))
                                        })
                                })
        ...
        if !filename.is_empty() {
            if filename.is_empty() {
                edit_state.validation_state =
                    ValidationState::Error("文件或文件夹名称不能为空。".to_string());
                cx.notify();
                return;
            }
    ...
    fn remove(
        ...
        maybe!({
            ...
            let answer = if !skip_prompt {
                let operation = if trash { "移至回收站" } else { "删除" };
                ...
                let prompt = match file_paths.first() {
                    ...
                    _ => {
                        ...
                        let unsaved_warning = if dirty_buffers == 0 {
                            ...
                            format!(
                                "\n\n其中 {dirty_buffers} 项存在未保存的更改，这些更改将会丢失。"
                            )
                        };

                        format!(
                            "{message_start} 以下 {} 个文件吗？\n{}{unsaved_warning}",
                            file_paths.len(),
                            names.join("\n")
                        )
                    }
