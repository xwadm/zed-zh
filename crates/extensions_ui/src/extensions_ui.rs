                ...
                window
                    .spawn(cx, async move |cx| {
                        ...
                        match install_task.await {
                            Ok(_) => {}
                            Err(err) => {
                                log::error!("开发扩展安装失败：{:?}", err);
                                workspace_handle
                                    .update(cx, |workspace, cx| {
                                        workspace.show_error(
                                            // 注意：此处使用 anyhow::context 会导致错误信息无法打印
                                            &format!("开发扩展安装失败：{}", err),
                                            cx,
                                        );
                                    })
...
fn extension_provides_label(provides: ExtensionProvides) -> &'static str {
    match provides {
        ...
        ExtensionProvides::ContextServers => "MCP服务器",
        ExtensionProvides::AgentServers => "代理服务器",
        ExtensionProvides::SlashCommands => "斜杠命令",
        ExtensionProvides::IndexedDocsProviders => "索引文档提供器",
        ...
        match status.clone() {
            ...
            ExtensionStatus::Installed(installed_version) => ExtensionCardButtons {
                ...
                .on_click({
                    let extension_id = extension.id.clone();
                    move |_, _, cx| {
                        telemetry::event!("扩展已卸载", extension_id);
                        ExtensionStore::global(cx).update(cx, |store, cx| {
                            store
                        ...
                        Button::new(extension_button_id(&extension.id, ExtensionOperation::Upgrade), "升级")
                          .style(ButtonStyle::Tinted(ui::TintColor::Accent))
                            ...
                            .on_click({
                                ...
                                let version = extension.manifest.version.clone();
                                move |_, _, cx| {
                                    telemetry::event!("扩展已安装", extension_id, version);
                                    ExtensionStore::global(cx).update(cx, |store, cx| {
                                        store
                                            .upgrade_extension(
                                                extension_id.clone(),
