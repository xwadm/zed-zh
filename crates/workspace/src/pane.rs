...
split_structs!(
    ...
    SplitUp => "向上分割面板。",
    SplitDown => "向下分割面板。",
    SplitHorizontal => "水平分割面板。",
    SplitVertical => "垂直分割面板。"
);
...
impl Pane {
    ...
    pub async fn save_item(
        ...
        const CONFLICT_MESSAGE: &str = "自你开始编辑后，该文件已在磁盘上被修改。是否要覆盖它？";

        const DELETED_MESSAGE: &str = "自你开始编辑后，该文件已在磁盘上被删除。是否要重新创建它？";
            ...
            IconButton::new("toggle_read_only", IconName::FileLock)
                ...
                .tooltip(move |_, cx| {
                    if toggleable {
                        ...
                        Tooltip::with_meta("文件已锁定", None, "该文件为只读", cx)
                    }
                })
    ...
    fn render_tab_bar(&mut self, window: &mut Window, cx: &mut Context<Pane>) -> AnyElement {
        ...
        if self.is_tab_pinned(tab_count) {
            log::warn!(
                "固定标签页数量({})超过实际标签页数量({})。\
                这种情况不应发生。如果可以，请在 https://github.com/zed-industries/zed/issues/33342 中\
                提供复现步骤",
                self.pinned_tab_count,
                tab_count
            );
...
mod tests {
    ...
    async fn test_reduce_max_tabs_closes_existing_items(cx: &mut TestAppContext) {
        ...
        add_labeled_item(&pane, "设置", false, cx);
    ...
    async fn test_separate_pinned_row_has_right_border(cx: &mut TestAppContext) {
        ...
        assert!(
            ...
            "双行布局中应存在 pinned_tabs_border 以显示右侧边框"
        );
    }
    ...
    async fn test_drag_pinned_tab_when_show_pinned_tabs_in_separate_row_enabled(
        ...
        let tab_a_bounds = cx
            .debug_bounds("TAB-0")
            .expect("标签 A（索引 0）应存在调试边界");
        let tab_c_bounds = cx
            .debug_bounds("TAB-2")
            .expect("标签 C（索引 2）应存在调试边界");
    ...
    async fn test_middle_click_pinned_tab_does_not_close(cx: &mut TestAppContext) {
        ...
        let tab_a_bounds = cx
            .debug_bounds("TAB-0")
            .expect("标签 A（索引 1）应存在调试边界");
        let tab_b_bounds = cx
            .debug_bounds("TAB-1")
            .expect("标签 B（索引 2）应存在调试边界");
    ...
    async fn test_double_click_pinned_tab_bar_empty_space_creates_new_tab(cx: &mut TestAppContext) {
        ...
        // TODO: 如果 workspace 测试中以后支持 editor::init，请移除此行
    ...
    async fn test_add_item_with_same_project_entries(cx: &mut TestAppContext) {
        ...
        assert_item_labels(&pane, ["缓冲区 1", "缓冲区 2", "多缓冲区 1*"], cx);
    ...
    async fn test_close_all_items(cx: &mut TestAppContext) {
        ...
        add_labeled_item(&pane, "干净1", false, cx);
        add_labeled_item(&pane, "已修改", true, cx).update(cx, |item, cx| {
            item.project_items
                .push(TestProjectItem::new_dirty(1, "已修改.txt", cx))
        });
        add_labeled_item(&pane, "干净2", false, cx);
        assert_item_labels(&pane, ["干净1", "已修改^", "干净2*"], cx);
    ...
    async fn test_discard_all_reloads_from_disk(cx: &mut TestAppContext) {
        ...
        item_a.read_with(cx, |item, _| {
            assert_eq!(item.reload_count, 1, "项目 A 应已重新加载");
            assert!(
                !item.is_dirty,
                "项目 A 重新加载后不应再处于已修改状态"
            );
        });
        item_b.read_with(cx, |item, _| {
            assert_eq!(item.reload_count, 1, "项目 B 应已重新加载");
            assert!(
                !item.is_dirty,
                "项目 B 重新加载后不应再处于已修改状态"
            );
        });
    }
    ...
    async fn test_dont_save_single_file_reloads_from_disk(cx: &mut TestAppContext) {
        ...
        item.read_with(cx, |item, _| {
            assert_eq!(item.reload_count, 1, "项目应已重新加载");
            assert!(
                !item.is_dirty,
                "项目重新加载后不应再处于已修改状态"
            );
    ...
    async fn test_discard_does_not_reload_multibuffer(cx: &mut TestAppContext) {
        ...
        singleton_item.update(cx, |item, cx| {
            item.project_items
                .push(TestProjectItem::new_dirty(1, "单例.txt", cx))
        });
        ...
        multi_item.update(cx, |item, cx| {
            item.project_items
                .push(TestProjectItem::new_dirty(2, "多例.txt", cx))
        });
        ...
        singleton_item.read_with(cx, |item, _| {
            assert_eq!(item.reload_count, 1, "单例项目应已重新加载");
            assert!(
                !item.is_dirty,
                "单例项目重新加载后不应再处于已修改状态"
            );
    ...
    async fn test_close_multibuffer_items(cx: &mut TestAppContext) {
        ...
        cx.executor().run_until_parked();
        cx.simulate_prompt_answer("取消全部");
        save.await.unwrap();
        assert_item_labels(&pane, ["C", "A*^"], cx);
    }
    ...
    async fn test_new_tab_scrolls_into_view_completely(cx: &mut TestAppContext) {
        ...
        assert!(
            ...
            "标签页不应与新建标签按钮重叠，若此断言失败，请检查界面是否经过重新设计！"
        );
    }
    ...
    async fn test_pinned_tabs_scroll_to_item_uses_correct_index(cx: &mut TestAppContext) {
        ...
        assert!(
            ...
            "测试需要标签页溢出以验证滚动功能。请增加标签页数量或减小窗口宽度。"
        );
        ...
        workspace.read_with(cx, |workspace, _| match &workspace.center.root {
            Member::Axis(axis) => {
                ...
                assert!(
                    ...
                    "面板 ID 与预期不匹配： {expected_ids:?} != {actual_ids:?}",
                    actual_ids = axis.members
                );
            }
            Member::Pane(_) => panic!("预期轴"),
        });
    }
