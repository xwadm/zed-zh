//! 用于管理 [`TrustedWorktrees`] 数据的用户界面接口
...
impl ModalView for SecurityModal {
    ...
    fn on_before_dismiss(&mut self, _: &mut Window, _: &mut Context<Self>) -> DismissDecision {
        match self.trusted {
            Some(false) => telemetry::event!("以受限模式打开", 来源 = "工作树信任弹窗"),
            Some(true) => telemetry::event!("信任并继续", 来源 = "工作树信任弹窗"),
            None => telemetry::event!("已关闭", 来源 = "工作树信任弹窗"),
        }
                ...
                v_flex()
                    ...
                    .child(
                        v_flex()
                            .child(
                                Label::new(
                                    "不受信任的项目将以受限模式打开，以保护你的系统安全。",
                                )
                                .color(Color::Muted),
                            )
                            .child(
                                Label::new(
                                    "请检查该项目配置的 .zed/settings.json 文件中的扩展或命令。",
                                )
                                .color(Color::Muted),
                            ),
                    )
                    .child(
                        v_flex()
                            ...
                            .child(ListBulletItem::new("安装MCP服务器集成组件")),
                    )
...
impl SecurityModal {
    ...
    fn build_trust_label(&self) -> Option<Cow<'static, str>> {
        ...
        match available_parents.len() {
            ...
            _ => Some(Cow::Borrowed("信任父级文件夹中的所有项目")),
        }
    }
