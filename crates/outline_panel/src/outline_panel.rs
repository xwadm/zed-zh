...
impl OutlinePanel {
    ...
    fn render_search_match(
        ...
        Some(self.entry_element(
            ...
            ElementId::from(SharedString::from(format!("搜索-{match_range:?}"))),
            depth,
            empty_icon(),
            is_active,
            entire_label,
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        .gap_1()
                        .justify_center()
                        .child(Label::new("切换面板关联").color(Color::Muted))
                        .child({
    ...
    fn render_filter_footer(&mut self, pinned: bool, cx: &mut Context<Self>) -> Div {
        let (pin_button_id, icon, icon_tooltip) = if pinned {
            ("取消固定按钮", IconName::Unpin, "取消固定大纲")
        } else {
            ("固定按钮", IconName::Pin, "固定当前大纲")
        };
    ...
    fn buffers_inside_directory(
        ...
        if !dir_entry.is_dir() {
            debug_panic!("buffers_inside_directory 被非目录条目调用 {dir_entry:?}");
    ...
    async fn test_project_search_results_toggling(cx: &mut TestAppContext) {
        ...
        let search_view = workspace.update_in(cx, |workspace, _window, cx| {
            workspace
                ...
                .expect("项目搜索视图应在新搜索事件触发后显示")
        });

        let query = "生命周期省略提示的参数名称";
      ...
      config.rs
        ...
        let select_first_in_all_matches = |line_to_select: &str| {
            assert!(
                ...
                "`{line_to_select}` 在所有匹配结果 `{all_matches}` 中未找到"
            );
    ...
    rust-analyzer/src/
      ...
      config.rs
        ...
        let select_first_in_all_matches = |line_to_select: &str| {
            assert!(
                all_matches.contains(line_to_select),
                "`{line_to_select}` 在所有匹配项 `{all_matches}` 中未找到"
            );
            all_matches.replacen(
                line_to_select,
                &format!("{line_to_select}{SELECTED_MARKER}"),
        ...
        let navigated_outline_selection =
                "搜索：Some(it) if config.«生命周期省略提示的参数名称» => {";
        outline_panel.update(cx, |outline_panel, cx| {
            assert_eq!(
                display_entries(
                    &project,
        ...
        let next_navigated_outline_selection = "搜索：InlayHintsConfig { «生命周期省略提示的参数名称»: true, ..TEST_CONFIG },";
        outline_panel.update(cx, |outline_panel, cx| {
            assert_eq!(
                display_entries(
                    &project,
...
大纲：fn 命名生命周期提示  <==== 已选中"
    ...
    async fn test_multiple_worktrees(cx: &mut TestAppContext) {
        ...
        let items = workspace
            ...
            .await;
        assert_eq!(items.len(), 1, "正在打开另一个工作树目录");
        assert!(
            items[0].is_none(),
            "目录应成功打开"
        );
