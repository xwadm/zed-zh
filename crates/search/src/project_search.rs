...
pub mod tests {
    ...
    async fn test_new_project_search_focus(cx: &mut TestAppContext) {
        ...
        window.update(cx, |_, window, cx| {
            search_view.update(cx, |search_view, cx| {
                    assert!(
                        search_view.query_editor.focus_handle(cx).is_focused(window),
                        "查询不匹配的搜索视图在切换焦点事件后应获得焦点：仍无结果面板可聚焦",
                    );
                });
        }).unwrap();
        ...
        window.update(cx, |_, window, cx| {
            search_view.update(cx, |search_view, cx| {
                    assert!(
                        ...
                        .focus_handle(cx).is_focused(window),
                        "查询匹配的搜索视图在切换焦点事件后，结果编辑器应仍保持聚焦状态",
                    );
                });
        }).unwrap();
        ...
        let Some(search_view_2) = cx.read(|cx| {
            workspace
                ...
                panic!("触发新搜索事件后应显示搜索视图")
            };
            assert!(
                search_view_2 != search_view,
                "触发 `workspace::NewSearch` 事件后应打开新的搜索视图"
            );
        ...
        window.update(cx, |_, window, cx| {
            search_view_2.update(cx, |search_view_2, cx| {
                    ...
                    "two",
                    "新搜索视图应从事件触发时文本光标所在位置（第一个搜索视图的第一条结果）获取查询内容"
                );
                assert_eq!(
                    search_view_2
                        .results_editor
    ...
    async fn test_search_query_history(cx: &mut TestAppContext) {
        ...
        window
            .update(cx, |_, window, cx| {
                search_view.update(cx, |search_view, cx| {
                    search_view.query_editor.update(cx, |query_editor, cx| {
                        query_editor.set_text("未保存的草稿", window, cx)
                    });
                })
            })
            .unwrap();
        ...
        window
            .update(cx, |_, _, cx| {
                search_view.update(cx, |search_view, cx| {
                    assert_eq!(search_view.query_editor.read(cx).text(cx), "未保存的草稿");
                });
            })
            .unwrap();
    }
    ...
    async fn test_buffer_search_query_reused(cx: &mut TestAppContext) {
        ...
        let buffer_search_query = "搜索栏查询内容";
        buffer_search_bar
    ...
    async fn test_search_with_inlays(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            requests_count.load(atomic::Ordering::Acquire),
            2,
            "清除旧项目搜索结果时已释放之前的缓冲区，因此会发起另一次查询",
        );
    ...
    async fn test_deleted_file_removed_from_search_results(cx: &mut TestAppContext) {
        ...
        search_view
            .update(cx, |search_view, _window, cx| {
                ...
                assert_eq!(match_count, 2, "应包含来自两个文件的匹配项");
            })
            .unwrap();
        ...
        search_view
            .update(cx, |search_view, _window, cx| {
                ...
                assert!(
                    ...
                    "已删除文件的内容应从搜索结果中移除，实际结果：{results_text}"
                );
                assert!(
                    results_text.contains("world"),
                    "剩余文件的内容应仍保留在结果中，实际结果：{results_text}"
                );
            })
            .unwrap();
        ...
        search_view
            .update(cx, |search_view, _window, cx| {
                ...
                assert!(
                    !results_text.contains("universe"),
                    "已删除的文件不应在重新搜索后重新出现，实际结果：{results_text}"
                );
                assert!(
                    results_text.contains("world"),
                    "剩余的文件仍应能被搜索到，实际结果：{results_text}"
                );
                assert_eq!(
        ...
        search_view.update_in(cx, |search_view, _window, cx| {
            assert!(
                !search_view
                    .search_options
                    .contains(SearchOptions::CASE_SENSITIVE),
                "智能大小写匹配应对全小写查询禁用大小写敏感，即使操作中显式设置了大小写敏感"
            );
            let query_text = search_view.query_editor.read(cx).text(cx);
