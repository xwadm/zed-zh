...
impl DebugPanel {
    ...
    pub fn start_session(
        ...
        session.update(cx, |session, _| match &mut session.state {
            ...
            SessionState::Running(_) => {
                debug_panic!("会话状态应处于构建中，因为我们刚刚启动它");
            }
        });
    }
        ...
        let project_path = match kind {
            TaskSourceKind::AbsPath { abs_path, .. } => {
                let Some(project_path) = workspace
                    ...
                    return Task::ready(Err(anyhow!("无绝对路径")));
                };
    ...
    pub(crate) fn save_scenario(
        ...
        self.workspace
            .update(cx, |workspace, cx| {
                ...
                return Task::ready(Err(anyhow!("无法获取工作树路径")));
                };
                ...
                cx.spawn_in(window, async move |workspace, cx| {
                    ...
                    let project_path = workspace.update(cx, |workspace, cx| {
                        workspace
                            ...
                            .context(
                                "无法获取当前工作树中 .zed/debug.json 的项目路径",
                            )
                    })??;
    ...
    pub fn insert_task_into_editor(
        ...
        static EMPTY_ARRAY_QUERY: LazyLock<Query> = LazyLock::new(|| {
            ...
            .expect("创建 EMPTY_ARRAY_QUERY 失败")
        });
                    ...
                    Some(ContextMenu::build(
                        ...
                        move |mut context_menu, _window, cx| {
                            ...
                            for (ix, _) in history.iter().enumerate().rev() {
                                context_menu =
                                    context_menu.entry(format!("历史记录-{}", ix + 1), None, {
                                        let running_state = running_state.clone();
                                        move |_window, cx| {
