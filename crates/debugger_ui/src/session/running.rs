...
impl Item for SubView {
    ...
    /// 用于序列化调试器窗格布局
    /// SharedString 在序列化/反序列化过程中与枚举相互转换。
...
impl RunningState {
    // todo(debugger) 将此方法移至 util，并改为传递一个将字符串转换的闭包
    pub(crate) fn substitute_variables_in_config(
        ...
        match config {
            ...
            serde_json::Value::String(s) => {
                // 某些内置 zed 任务会用引号包裹参数，因为它们可能包含空格。
    ...
    pub(crate) fn resolve_scenario(
        ...
        let Some(workspace) = self.workspace.upgrade() else {
            return Task::ready(Err(anyhow!("没有工作区")));
        };
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            if Self::contains_substring(&config, PROCESS_ID_PLACEHOLDER.as_str()) || label.as_ref().contains(PROCESS_ID_PLACEHOLDER.as_str()) {
                ...
                let Some(process_id) = rx.await.ok().flatten() else {
                    bail!("未选择进程，但配置中包含 {}", PROCESS_ID_PLACEHOLDER.as_str())
                };
            ...
            let request_type = match dap_registry
                .adapter(&adapter)
                .with_context(|| format!("{}：不是有效的适配器名称", &adapter)) {
            ...
            if config_is_valid {
                ...
                let locator_name =
                    locator_name.with_context(|| {
                        format!("找不到有效的构建任务定位器，且配置无效，错误信息：{}", request_type.err()
                ...
                let scenario = dap_registry
                    .adapter(&adapter)
                    .with_context(|| anyhow!("{}：不是有效的适配器名称", &adapter))?.config_from_zed_format(zed_config)
                    .await?;
                ...
                anyhow::bail!("Zed 无法确定如何运行此调试场景。未提供 `build` 字段，且调试适配器因以下原因不接受提供的配置：{e}");
            };
    ...
    fn handle_run_in_terminal(
        ...
        let command = if envs.contains_key("VSCODE_INSPECTOR_OPTIONS") {
            // 处理 NodeJS 调试适配器的特殊情况
            // 如果提供了 Node 二进制路径（可能带有 --experimental-network-inspection 等参数），
            // 我们将命令设置为 None
            // 这样可以防止出现 NodeJS REPL，这不是预期的行为
            // 预期用法是让用户提供自己的 Node 命令，例如 `node test.js`
            // 这允许 NodeJS 调试客户端正确附加
            if args
                .iter()
