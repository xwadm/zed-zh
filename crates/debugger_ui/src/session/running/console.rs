...
impl Console {
    pub fn new(
        ...
        let query_bar = cx.new(|cx| {
            ...
            editor.set_placeholder_text("输入表达式以求值", window, cx);
            editor.set_use_autoclose(false);
            editor.set_show_gutter(false, cx);
            editor.set_show_wrap_guides(false, cx);
            editor.set_show_indent_guides(false, cx);
    ...
    let color_fetcher: fn(&Theme) -> Hsla = match color {
        ...
        ansi::Color::Indexed(i) => {
            match i {
                ...
                // 16-231 是一个 6×6×6 的 RGB 色彩立方体，使用 XTerm 定义的步长映射到 0-255。
                // 参考：https://github.com/xterm-x11/xterm-snapshots/blob/master/256colres.pl
                // 16..=231 => {
                ...
                //     )
                // }
                // 232-255 是从 (8, 8, 8) 到 (238, 238, 238) 的 24 级灰度渐变。
                // 232..=255 => {
                //     let i = index as u8 - 232; // 将索引对齐到 0..24
                //     let value = i * 10 + 8;
                //     rgba_color(value, value, value)
                // }
                // 为了与 alacritty::Colors 接口兼容
                // 参考：https://github.com/alacritty/alacritty/blob/master/alacritty_terminal/src/term/color.rs
                _ => |_| gpui::black(),
            }
        }
    };
