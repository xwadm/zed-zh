...
/// TODO(设置界面): 在非开发构建中（`#[cfg(not(debug_assertions))]`）将此渲染为“在JSON中编辑”
#[derive(Clone, Copy)]
...
impl std::fmt::Debug for SettingsPageItem {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
        SettingsPageItem::SectionHeader(header) => write!(f, "分区标题({})", header),
        SettingsPageItem::SettingItem(setting_item) => {
            write!(f, "设置项({})", setting_item.title)
        }
        SettingsPageItem::SubPageLink(sub_page_link) => {
            write!(f, "子页面链接({})", sub_page_link.title)
        }
        SettingsPageItem::DynamicItem(dynamic_item) => {
            write!(f, "动态项({})", dynamic_item.discriminant.title)
        }
        SettingsPageItem::ActionLink(action_link) => {
            write!(f, "操作链接({})", action_link.title)
        }
    }
    }
}
    ...
    fn render(
            ...
            |setting_item: &SettingItem,
             ...
             cx: &mut Context<SettingsWindow>| {
                ...
                let field_renderer_or_warning =
                    field_renderer.ok_or("未找到渲染器").and_then(|renderer| {
        ...
        match self {
            ...
            SettingsPageItem::DynamicItem(DynamicItem {
                ...
                if rendered_ok {
                    let discriminant =
                        discriminant.expect("若 rendered_ok 为 true，则此值必定为 Some");
...
fn all_language_names(cx: &App) -> Vec<SharedString> {
    ...
    state
        ...
        .language_names()
        .into_iter()
        .filter(|name| name.as_ref() != "Zed 快捷键上下文")
        .map(Into::into)
        .collect()
}
...
impl SettingsUiFile {
    ...
    fn from_settings(file: settings::SettingsFile) -> Option<Self> {
        Some(match file {
            ...
            settings::SettingsFile::Server => SettingsUiFile::Server("待办：服务器名称"),
...
impl SettingsWindow {
    ...
    fn update_matches(&mut self, cx: &mut Context<SettingsWindow>) {
        ...
        self.search_task = Some(cx.spawn(async move |this, cx| {
            ...
            telemetry::event!("设置界面已搜索", query = query)
        }));
    }
    ...
    fn render_no_results(&self, cx: &App) -> impl IntoElement {
        ...
        v_flex()
            ...
            .child(Label::new("无结果"))
            .child(
                Label::new(format!("未找到与 \"{}\" 匹配的设置", search_query))
                    .size(LabelSize::Small)
                    .color(Color::Muted),
            )
    }
            ...
            SettingsStore::global(cx).error_for_file(self.current_file.to_settings())
        {
            fn banner(
                ...
                if shown_errors.insert(error.clone()) {
                    telemetry::event!("设置界面错误已显示", label = label, error = &error);
                }
                Banner::new()
...
fn update_settings_file(
    ...
    telemetry::event!("设置已更改", 配置项 = file_name, 类型 = file.setting_type());
    match file {
...
fn update_project_setting_file(
    ...
    let Some((worktree, project)) =
        ...
        anyhow::bail!("无法找到具有工作树 ID 的项目: {}", worktree_id);
    };
    ...
    Switch::new("toggle_button", toggle_state)
        ...
        .on_click({
            move |state, window, cx| {
                telemetry::event!("设置已更改", 配置项 = field.json_path, 类型 = file.setting_type());
...
pub mod test {
    ...
    fn parse(input: &'static str, window: &mut Window, cx: &mut App) -> SettingsWindow {
            ...
            .filter(|line| !line.is_empty())
        {
            ...
            assert!(selected_idx.is_none(), "只允许选中一个条目");
        ...
        let mut settings_window = SettingsWindow {
            ...
            navbar_entry: selected_idx.expect("必须选中一个导航栏条目"),
            navbar_entries: Vec::default(),
            navbar_scroll_handle: UniformListScrollHandle::default(),
            navbar_focus_subscriptions: vec![],
            filter_table: vec![],
    ...
    async fn test_settings_window_updates_when_new_workspace_created(
        ...
        settings_window.read_with(cx, |settings_window, _| {
            ...
            assert!(
                worktree_names.iter().any(|name| name == "worktree_a"),
                "应包含 worktree_a，但实际找到：{:?}",
                worktree_names
            );
            assert!(
                worktree_names.iter().any(|name| name == "worktree_b"),
                "应包含新创建工作区中的 worktree_b，但实际找到：{:?}",
                worktree_names
            );
...
mod project_settings_update_tests {
    ...
    async fn test_updates_are_serialized(cx: &mut TestAppContext) {
        ...
        assert_eq!(order, vec![1, 2, 3], "更新应按顺序处理");
        let buffer_store = setup
            .project
            .read_with(cx, |project, _| project.buffer_store().clone());
        let buffer = buffer_store
    ...
    async fn test_reloads_conflicted_buffer(cx: &mut TestAppContext) {
        ...
        assert!(has_unsaved_edits, "缓冲区应存在未保存的编辑");
        ...
        assert!(
            text.contains("\"tab_size\": 4"),
            "重新加载并更新后，缓冲区应包含新的 tab_size：{}",
            text
        );
        assert!(
            !text.contains("// comment"),
            "重新加载后，缓冲区不应包含未保存的编辑内容：{}",
            text
        );
        assert!(
            !text.contains("99"),
