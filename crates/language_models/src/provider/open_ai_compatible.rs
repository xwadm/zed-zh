    ...
    fn render(&mut self, _: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let api_key_section = if self.should_render_editor(cx) {
            v_flex()
                ...
                .child(Label::new("要将 Zed 代理与兼容 OpenAI 的提供商一起使用，你需要添加 API 密钥。"))
                .child(
                    div()
                ...
                .child(
                    Label::new(
                        format!("你也可以设置 {env_var_name} 环境变量并重启 Zed。"),
                    )
                        ...
                        .child(
                            Button::new("reset-api-key", "重置 API 密钥")
                                ...
                                .when(env_var_set, |this| {
                                    this.tooltip(Tooltip::text(format!("要重置你的 API 密钥，请取消设置 {env_var_name} 环境变量。")))
                                })
