...
impl Buffer {
    ...
    pub fn from_proto(
        ...
        let buffer_id = BufferId::new(message.id).context("无法反序列化缓冲区 ID")?;
        let buffer = TextBuffer::new(replica_id, buffer_id, message.base_text);
        let mut this = Self::build(buffer, file, capability);
        this.text.set_line_ending(proto::deserialize_line_ending(
            rpc::proto::LineEnding::from_i32(message.line_ending).context("缺少 line_ending 字段")?,
        ));
...
impl Buffer {
        ...
        T: rand::Rng,
    {
        ...
        log::info!("正在通过 {:?} 变更缓冲区 {:?}", self.replica_id(), edits);
        self.edit(edits, None, cx);
    }
        ...
        if let Some(highlights) = self.highlights.as_mut() {
            if old_range.start <= self.range.start && old_range.end >= self.range.end {
                ...
                *highlights = BufferChunkHighlights {
                ...
                debug_assert!(
                    false,
                    "尝试在无关联缓冲区快照的情况下，对支持语言感知的缓冲区迭代器执行查找操作"
                );
            }
