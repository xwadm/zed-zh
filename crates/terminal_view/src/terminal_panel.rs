...
impl TerminalPanel {
                            ...
                            .menu(move |window, cx| {
                                ...
                                let menu = ContextMenu::build(window, cx, |menu, _, _| {
                                    menu.context(focus_handle.clone())
                                        ...
                                        // 我们希望任务模态框关闭后焦点回到终端面板，
                                        // 因此先聚焦终端面板。否则，一旦打开模态框，
                                        // 上下文菜单消失，就没有聚焦元素了。
                                        .action(
                                            "生成任务",
    ...
    pub async fn load(
        ...
        // 面板/停靠区在工作区之外加载，因此我们在这里进行清理，而不是通过工作区。
        if let Some(workspace) = workspace.upgrade() {
    ...
    pub fn spawn_task(
        ...
        if project.is_via_collab() {
            return Task::ready(Err(anyhow!("协作项目暂不支持终端功能")));
        }
    ...
    pub fn add_center_terminal(
        ...
        if !is_enabled_in_workspace(workspace, cx) {
            return Task::ready(Err(anyhow!("远程项目暂不支持终端功能")));
        }
                ...
                RevealStrategy::Always => match reveal_target {
                    RevealTarget::Center => {
                        task_workspace.update_in(cx, |workspace, window, cx| {
                            ...
                            anyhow::ensure!(did_activate, "无法获取终端窗格");
...
/// 根据 Shell 配置计算命令、参数和命令标签，准备用于生成的 `SpawnInTerminal`。
/// 这是一个纯函数，可以在不生成实际终端的情况下进行测试。
...
mod tests {
    ...
    async fn test_bypass_max_tabs_limit(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            item_count, 5,
            "终端面板应绕过 max_tabs 限制并显示全部 5 个终端"
        );
    }
    ...
    async fn test_new_terminal_opens_in_center_when_center_terminal_focused(
        ...
        let center_items_before = window_handle
            ...
            .expect("读取中央窗格内容失败");
        assert_eq!(center_items_before, 1, "中央窗格应包含 1 个终端");
    ...
    async fn test_new_local_terminal_opens_in_center_when_center_terminal_focused(
        ...
        window_handle
            ...
            .expect("派发 new_terminal with local=true 命令失败");
        cx.run_until_parked();
