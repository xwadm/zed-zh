...
impl KeymapEditor {
    ...
    fn render_no_matches_hint(&self, _window: &mut Window, _cx: &App) -> AnyElement {
        ...
        (FilterState::All, SearchMode::Normal) => "未找到匹配查询的内容",
    };
    ...
    fn open_edit_keybinding_modal(
        ...
        let context = keybind
            .context()
            .map(|context| context.local_str().unwrap_or("全局"));
    ...
    fn copy_context_to_clipboard(
        ...
        telemetry::event!("快捷键上下文已复制", context = context);
    ...
    fn copy_action_to_clipboard(
        ...
        telemetry::event!("快捷键操作已复制", action = action);
                        ...
                        h_flex()
                            ...
                            .child(
                                h_flex()
                                    ...
                                    .child(
                                        Button::new("edit-in-json", "JSON 编辑")
                                            ...
                                            |row, conflict| {
                                                ...
                                                let context = overriding_binding.and_then(|binding| {
                                                    match conflict.override_source {
                                                        KeybindSource::User  => Some("你的快捷键配置"),
                                                        KeybindSource::Vim => Some("Vim 快捷键配置"),
                                                        KeybindSource::Base => Some("基础快捷键配置"),
                                                        _ => {
                                                            log::error!("意外的覆盖来源：{} 快捷键配置", conflict.override_source.name());
                                                            None
                                                        }
                                                    }.map(|source| format!("该快捷键被来自 {} 的“{}”绑定覆盖。", binding.action().humanized_name, source))
                                                }).unwrap_or_else(|| "该绑定已被覆盖。".to_string());
...
impl KeybindingEditorModal {
    ...
    fn validate_action_arguments(&self, cx: &App) -> anyhow::Result<Option<String>> {
        ...
        let value = action_arguments
            .as_ref()
            .map(|args| {
                serde_json::from_str(args).context("解析操作参数为 JSON 失败")
            })
            .transpose()?;
    ...
    fn validate_keystrokes(&self, cx: &App) -> anyhow::Result<Vec<KeybindingKeystroke>> {
        ...
        anyhow::ensure!(!new_keystrokes.is_empty(), "快捷键不能为空");
        Ok(new_keystrokes)
    }
    ...
    fn validate_context(&self, cx: &App) -> anyhow::Result<Option<String>> {
        ...
        gpui::KeyBindingContextPredicate::parse(&context).context("解析快捷键上下文失败")?;
    ...
    if remaining_conflict_amount > 0 {
        format!(
            "你的快捷键将与“{}”操作以及 {} 个其他绑定冲突",
            name, remaining_conflict_amount
        )
    } else {
        format!("你的快捷键将与“{}”操作冲突", name)
    }
}
None => {
    log::info!(
...
async fn load_json_language(workspace: WeakEntity<Workspace>, cx: &mut AsyncApp) -> Arc<Language> {
    let json_language_task = workspace
        ...
        .context("加载 JSON 语言失败")
        .log_err();
    let json_language = match json_language_task {
        Some(task) => task.await.context("加载 JSON 语言失败").log_err(),
        None => None,
    };
    ...
    let language_task = workspace
        .read_with(cx, |workspace, cx| {
            workspace
                ...
                .read(cx)
                .languages()
                .language_for_name("Zed 快捷键上下文")
        })
        .context("加载 Zed 快捷键上下文语言失败")
        .log_err();
...
async fn save_keybinding_update(
    ...
    .map_err(|err| anyhow::anyhow!("无法保存更新后的快捷键：{}", err))?;
    fs.write(
...
impl SerializableItem for KeymapEditor {
    ...
    fn cleanup(
        ...
        workspace::delete_unloaded_items(alive_items, workspace_id, "快捷键编辑器", &db, cx)
    }
