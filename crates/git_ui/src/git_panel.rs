...
fn git_panel_context_menu(
    ...
    ContextMenu::build(window, cx, move |context_menu, _, _| {
        context_menu
            ...
            .action_disabled_when(!state.has_stash_items, "恢复储藏", StashPop.boxed_clone())
            .action("查看储藏", zed_actions::git::ViewStash.boxed_clone())
            .separator()
            .action("打开差异对比", project_diff::Diff.boxed_clone())
            .separator()
            .action_disabled_when(
...
impl GitPanel {
            ...
            cx.spawn(async move |_, cx| {
                if let Err(e) = receiver.await? {
                    if let Some(workspace) = workspace.upgrade() {
                        cx.update(|cx| {
                            show_error_toast(workspace, "添加到 .gitignore 失败", e, cx);
                        });
                    }
                }
                anyhow::Ok(())
    ...
    fn revert_entry(
        ...
        maybe!({
            ...
            if !entry.status.is_created() {
                ...
                let prompt = prompt(&format!("将 {} 移入回收站？", filename), None, window, cx);
        ...
        cx.spawn_in(window, async move |this, cx| {
            ...
            this.update_in(cx, |this, window, cx| {
                if let Err(err) = result {
                    this.update_visible_entries(window, cx);
                    this.show_error_toast("切换分支失败", err, cx);
                }
            })
            .ok();
        })
    ...
    fn clean_all(&mut self, _: &TrashUntrackedFiles, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if to_delete.len() > 5 {
            details.push_str(&format!("\n以及其他 {} 项…", to_delete.len() - 5))
        }

        let prompt = prompt("将这些文件移入回收站？", Some(&details), window, cx);
        cx.spawn_in(window, async move |this, cx| {
            match prompt.await? {
                TrashCancel::Trash => {}
                TrashCancel::Cancel => return Ok(()),
        ...
        .detach_and_prompt_err("无法将文件移入回收站", window, cx, |e, _, _| {
            Some(format!("{e}"))
        });
    }
        ...
        cx.spawn({
            async move |this, cx| {
                ...
                this.update(cx, |this, cx| {
                    if let Err(err) = result {
                        this.show_error_toast(if stage { "暂存文件失败" } else { "取消暂存失败" }, err, cx);
                    }
                    cx.notify()
                })
            }
    ...
    fn stage_status_for_directory(
        ...
        util::debug_panic!("在平铺视图模式下，不应渲染目录项");
        ...
        cx.spawn({
            async move |this, cx| {
                ...
                this.update(cx, |this, cx| {
                    stash_task
                        .map_err(|e| {
                            this.show_error_toast("恢复储藏失败", e, cx);
                        })
                        .ok();
                    cx.notify();
                })
        ...
        cx.spawn({
            async move |this, cx| {
                ...
                this.update(cx, |this, cx| {
                    stash_task
                        .map_err(|e| {
                            this.show_error_toast("应用储藏失败", e, cx);
                        })
                        .ok();
                    cx.notify();
                })
    ...
    fn on_commit(&mut self, _: &git::Commit, window: &mut Window, cx: &mut Context<Self>) {
        if self.commit(&self.commit_editor.focus_handle(cx), window, cx) {
            telemetry::event!("Git_已提交", 来源 = "Git面板");
        }
    }
    ...
    fn on_amend(&mut self, _: &git::Amend, window: &mut Window, cx: &mut Context<Self>) {
        if self.amend(&self.commit_editor.focus_handle(cx), window, cx) {
            telemetry::event!("Git_已修改提交", 来源 = "Git面板");
        }
    }
                ...
                Ok(None) => {
                    this.update(cx, |this, cx| {
                        this.show_error_toast(
                            "push",
                            anyhow::anyhow!("没有可推送的远程仓库。请添加一个远程仓库以发布更改。"),
                            cx,
                        )
                    })
                    .ok();
    ...
    pub fn create_pull_request(&self, window: &mut Window, cx: &mut Context<Self>) {
        let result = (|| -> anyhow::Result<()> {
            let repo = self
                .active_repository
                .clone()
                .ok_or_else(|| anyhow::anyhow!("无可用仓库"))?;
            ...
            let branch = branch.ok_or_else(|| anyhow::anyhow!("无活跃分支"))?;
            ...
            let Some((provider, parsed_remote)) =
                ...
                return Err(anyhow::anyhow!("不支持的远程仓库链接: {}", remote_url));
            };

            let Some(url) = provider.build_create_pull_request_url(&parsed_remote, &source_branch)
            else {
                return Err(anyhow::anyhow!("无法构建拉取请求链接"));
            };
        ...
        if let Err(err) = result {
            log::error!("创建拉取请求时出错 {:?}", err);
            cx.defer_in(window, |panel, _window, cx| {
                panel.show_error_toast("创建拉取请求失败", err, cx);
            });
        }
    }
    ...
    fn toggle_directory(&mut self, key: &TreeKey, window: &mut Window, cx: &mut Context<Self>) {
        if let Some(state) = self.view_mode.tree_state_mut() {
            ...
            *expanded = !*expanded;
            ...
            util::debug_panic!("尝试在扁平 Git 面板状态下切换目录");
        }
    }
    ...
    fn update_visible_entries(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let placeholder_text = suggested_commit_message.unwrap_or("请输入提交信息".into());
    ...
    pub(crate) fn render_generate_commit_message_button(
        ...
        Some(
            IconButton::new("generate-commit-message", IconName::AiEdit)
                ...
                if !can_commit {
                    Tooltip::simple("暂无更改可提交", cx)
                } else if has_commit_model_configuration_error {
                    Tooltip::simple("请配置大语言模型提供商以生成提交信息", cx)
                } else {
            ...
            .when(title_exceeds_limit, |this| {
                this.child(
                    h_flex()
                        ...
                        .child(
                            Label::new(format!(
                                "提交信息标题超出 {max_title_length} 字符限制。"
                            ))
                            .size(LabelSize::Small),
                        ),
                )
        ...
        div()
            ...
            .child(SplitButton::new(
                ...
                .on_click({
                    let git_panel = cx.weak_entity();
                    move |_, window, cx| {
                        telemetry::event!("Git 已提交", 来源 = "Git 面板");
                        git_panel
                            .update(cx, |git_panel, cx| {
                    ...
                    h_flex()
                                ...
                                panel_icon_button("undo", IconName::Undo)
                                    ...
                                    .tooltip(move |_window, cx| {
                                        Tooltip::with_meta(
                                            ...
                                            Some(&git::Uncommit),
                                            if has_unstaged {
                                                "git reset HEAD^ --soft（保留修改）"
                                            } else {
                                                "git reset HEAD^（不保留修改）"
                                            },
                                            cx,
                                        )
                                    })
                        ...
                        .child(
                            panel_icon_button("git-graph-button", IconName::GitGraph)
                                .icon_size(IconSize::Small)
                                .tooltip(|_window, cx| {
                                    Tooltip::for_action("打开 Git 提交图", &Open, cx)
                                })
    ...
    fn render_directory_entry(
        ...
        // 生成目录复选框的唯一元素 ID
        let checkbox_id: ElementId =
            ElementId::Name(format!("目录复选框_{}_{}", entry.name, ix).into());

        // 生成目录复选框容器的唯一元素 ID
        let checkbox_wrapper_id: ElementId =
            ElementId::Name(format!("目录复选框容器_{}_{}", entry.name, ix).into());
        let selected_bg_alpha = 0.08;
        let state_opacity_step = 0.04;
        ...
        .children(vec![
            example_group_with_title(
                ...
                vec![
                    ...
                    single_example(
                        "超前且落后于远程", // Ahead and behind remote
                        div()
                            .w(example_width)
                            .overflow_hidden()
                            .child(PanelRepoFooter::new_preview(
...
fn open_output(
    ...
    let editor = cx.new(|cx| {
        ...
        editor.buffer().update(cx, |buffer, cx| {
            buffer.set_title(format!("Git {operation} 命令输出结果"), cx);
        });
...
mod tests {
    ...
    #[test]
    /// 测试 Git 错误提示格式化：错误被包装时仍优先使用原始 RPC 信息
    fn test_format_git_error_toast_message_prefers_raw_rpc_message_when_wrapped() {
        ...
        // 包装一层上下文：发送拉取请求
        let wrapped = rpc_error.context("发送拉取请求");
    ...
    async fn test_amend_commit_message_handling(cx: &mut TestAppContext) {
        ...
        panel.update(cx, |panel, cx| {
            panel.commit_message_buffer(cx).update(cx, |buffer, cx| {
                let start = buffer.anchor_before(0);
                let end = buffer.anchor_after(buffer.len());
                buffer.edit([(start..end, "初始提交信息")], None, cx);
            });
            ...
            assert_eq!(current_message, "初始提交信息");
        ...
        panel.update(cx, |panel, cx| {
            ...
            panel.commit_message_buffer(cx).update(cx, |buffer, cx| {
                let start = buffer.anchor_before(0);
                let end = buffer.anchor_after(buffer.len());
                buffer.edit([(start..end, "上一次提交信息")], None, cx);
            });
    ...
    async fn test_amend(cx: &mut TestAppContext) {
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            panel.commit_message_buffer(cx).update(cx, |buffer, cx| {
                buffer.set_text("重构: 更新 main.rs 文件", cx);
            });
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            assert_eq!(
                ...
                Some("重构: 更新 main.rs 文件".to_string())
            );
    ...
    async fn test_open_diff(cx: &mut TestAppContext) {
        ...
        workspace.update_in(cx,|workspace, _window, cx| {
            ...
            // 断言当前激活路径为未跟踪文件
            assert_eq!(active_path.path, rel_path("未跟踪文件").into_arc());
        });
    }
    ...
    async fn test_suggest_commit_message(cx: &mut TestAppContext) {
        ...
        assert_eq!(message, Some("更新已跟踪文件".to_string()));
        ...
        // "Update untracked"
        let message = panel.update(cx, |panel, cx| panel.suggest_commit_message(cx));
        assert_eq!(message, Some(""创建未跟踪文件"".to_string()));
    ...
    async fn test_dispatch_context_with_focus_states(cx: &mut TestAppContext) {
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            assert!(
                editor_is_focused,
                "执行聚焦编辑器操作后，提交编辑器应获得焦点"
            );
            let context = panel.dispatch_context(window, cx);
            assert!(
                context.contains("Git面板"),
                "应始终包含 Git面板 上下文"
            );
            assert!(
                context.contains("提交编辑器"),
                "提交编辑器获得焦点时应包含 提交编辑器 上下文"
            );
            assert!(
                !context.contains("菜单"),
                "提交编辑器获得焦点时不应包含 菜单 上下文"
            );
            assert!(
                !context.contains("变更列表"),
                "提交编辑器获得焦点时不应包含 变更列表 上下文"
            );
        });
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            assert!(
                context.contains("变更列表"),
                "变更列表获得焦点时应包含 变更列表 上下文"
            );
            assert!(
                !context.contains("提交编辑器"),
                "变更列表获得焦点时不应包含 提交编辑器 上下文"
            );
        });
        ...
        panel.update_in(cx, |panel, window, cx| {
            ...
            assert!(
                context.contains("提交编辑器"),
                "将焦点切回编辑器后，应包含 提交编辑器 上下文"
            );
            assert!(
                !context.contains("菜单"),
                "将焦点切回编辑器后，不应包含 菜单 上下文"
            );
        });
