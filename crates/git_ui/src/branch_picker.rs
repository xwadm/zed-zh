            ...
            IconButton::new(("delete", entry_ix), IconName::Trash)
                ...
                .on_click(cx.listener(move |this, _, window, cx| {
                    ...
                    let create_from_default_button = self.default_branch.as_ref().map(|default_branch| {
                        let tooltip_label: SharedString = format!("从以下位置新建：{default_branch}").into();
                        let focus_handle = self.focus_handle.clone();
        ...
        match self.state {
            PickerState::List => {
                ...
                let branch_from_default_button = self
                    ...
                    .map(|default_branch| {
                        let button_label = format!("从{default_branch}新建");
            ...
            PickerState::NewBranch => {
                let branch_from_default_button =
                    self.default_branch.as_ref().map(|default_branch| {
                        let button_label = format!("从{default_branch}创建新分支");
...
mod tests {
    ...
    fn create_test_branches() -> Vec<Branch> {
        vec![
            create_test_branch("主分支", true, None, Some(1000)),
            create_test_branch("功能-认证", false, None, Some(900)),
            create_test_branch("功能-界面", false, None, Some(800)),
            create_test_branch("开发分支", false, None, Some(700)),
        ]
    }
    ...
    async fn test_delete_branch(cx: &mut TestAppContext) {
        ...
        let expected_branches = ["主分支", "功能-认证", "功能-界面", "开发分支"]
            .into_iter()
    ...
    async fn test_delete_remote_branch(cx: &mut TestAppContext) {
        ...
        let (_project, repository) = init_fake_repository(cx).await;
        let branches = vec![
            create_test_branch("主分支", true, Some("origin"), Some(1000)),
            create_test_branch("功能-认证", false, Some("origin"), Some(900)),
            create_test_branch("功能-界面", false, Some("fork"), Some(800)),
            create_test_branch("开发分支", false, Some("private"), Some(700)),
        ];
    ...
    async fn test_branch_filter_shows_all_then_remotes_and_applies_query(cx: &mut TestAppContext) {
        ...
        let branches = vec![
            create_test_branch("主分支", true, Some("origin"), Some(1000)),
            create_test_branch("功能-认证", false, Some("fork"), Some(900)),
            create_test_branch("功能-界面", false, None, Some(800)),
            create_test_branch("开发分支", false, None, Some(700)),
        ];
        ...
        branch_list.update(cx, |branch_list, cx| {
            branch_list.picker.update(cx, |picker, _cx| {
                ...
                assert_eq!(
                    branches,
                    ["origin/主分支", "fork/功能-认证", "功能-界面", "开发分支"]
                        .into_iter()
                        .collect::<HashSet<_>>()
                );
                ...
                assert_eq!(
                    ordered,
                    vec!["功能-界面", "开发分支", "origin/主分支", "fork/功能-认证"]
                );
    ...
    async fn test_new_branch_creation_with_query(test_cx: &mut TestAppContext) {
        const MAIN_BRANCH: &str = "主分支";
        const FEATURE_BRANCH: &str = "功能分支";
        const NEW_BRANCH: &str = "新功能分支";
        ...
        branch_list
            .update_in(cx, |branch_list, window, cx| {
                branch_list.picker.update(cx, |picker, cx| {
                    ...
                    picker
                        .delegate
                        .update_matches("我的新远程仓库".to_string(), window, cx)
                })
            })
            .await;
