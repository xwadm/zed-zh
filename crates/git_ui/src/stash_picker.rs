...
impl StashListDelegate {
    ...
    fn drop_stash_at(&self, ix: usize, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        .detach_and_prompt_err("删除储藏失败", window, cx, |e, _, _| {
            Some(e.to_string())
        });
    }
    ...
    fn pop_stash(&self, stash_index: usize, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        .detach_and_prompt_err("恢复储藏失败", window, cx, |e, _, _| {
            Some(e.to_string())
        });
        cx.emit(DismissEvent);
    }
    ...
    fn apply_stash(&self, stash_index: usize, window: &mut Window, cx: &mut Context<Picker<Self>>) {
        ...
        .detach_and_prompt_err("应用储藏失败", window, cx, |e, _, _| {
            Some(e.to_string())
        });
        cx.emit(DismissEvent);
    }
    ...
    fn render_match(
        ...
        let pop_button = {
            ...
            IconButton::new(("pop-stash", ix), IconName::MaximizeAlt)
                .icon_size(IconSize::Small)
                .tooltip(move |_, cx| {
                    Tooltip::for_action_in("恢复储藏", &menu::SecondaryConfirm, &focus_handle, cx)
                })
        ...
        let drop_button = {
            ...
            IconButton::new(("drop-stash", ix), IconName::Trash)
                .icon_size(IconSize::Small)
                .tooltip(move |_, cx| {
                    Tooltip::for_action_in("删除储藏", &DropStashItem, &focus_handle, cx)
                })
                .on_click(cx.listener(move |this, _, window, cx| {
...
mod tests {
    ...
    async fn test_show_stash_dismisses(cx: &mut TestAppContext) {
        ...
        let stash_entries = vec![
            stash_entry(0, "储藏 #0", Some("主分支")),
            stash_entry(1, "储藏 #1", Some("开发分支")),
        ];
