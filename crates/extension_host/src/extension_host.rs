...
impl ExtensionStore {
    ...
    pub fn fetch_extensions_with_update_available(
        ...
        let task = self.fetch_extensions_from_api(
            "/extensions/updates",
            &[
                    ("最小架构版本", &schema_versions.start().to_string()),
                    ("最大架构版本", &schema_versions.end().to_string()),
                    (
                        "最小WASM接口版本",
                        &wasm_api_versions.start().to_string(),
                    ),
                    ("最大WASM接口版本", &wasm_api_versions.end().to_string()),
                    ("编号", &extension_ids),
                ],
            cx,
        );
        ...
        cx.spawn(async move |this, cx| {
            ...
            if let Some(content_length) = content_length {
                ...
                if content_length != actual_len {
                    bail!(concat!(
                        "下载的扩展大小 {actual_len} 与内容长度 {content_length} 不匹配",
                    ));
                }
            }
    ...
    pub fn install_latest_extension(&mut self, extension_id: Arc<str>, cx: &mut Context<Self>) {
        log::info!("正在安装扩展 {extension_id} 的最新版本");
        ...
        let Some(url) = self
            ...
            .build_zed_api_url(
                ...
                &[
                        ...
                        ("最大WASM应用程序接口版本", &wasm_api_versions.end().to_string()),
                    ],
            )
            .log_err()
        else {
    ...
    fn install_or_upgrade_extension(
        ...
        log::info!("正在安装扩展 {extension_id} {version}");
        let Some(url) = self
            .http_client
            .build_zed_api_url(
    ...
    pub fn uninstall_extension(
        ...
        cx.spawn(async move |extension_store, cx| {
            ...
            .await
            .with_context(|| format!("移除扩展目录 {extension_dir:?} 失败"))?;
            ...
            for i in 0..3 {
                ...
                match removal_result {
                    ...
                    Err(e) => {
                        if i == 2 {
                            log::error!("移除扩展工作目录 {work_dir:?} 失败：{e}");
                        }
                    }
                }
            }
    ...
    fn extensions_updated(
        ...
        cx.spawn(async move |this, cx| {
                ...
                async move {
                    ...
                    for snippets_path in &snippets_to_add {
                        match fs
                            .load(snippets_path)
                            .await
                            .with_context(|| format!("从 {snippets_path:?} 加载代码片段失败"))
                        {
            ...
            for extension in extension_entries {
                ...
                .await
                .with_context(|| format!("从 {extension_path:?} 加载扩展失败"));
    ...
    fn rebuild_extension_index(&self, cx: &mut Context<Self>) -> Task<ExtensionIndex> {
        ...
        cx.background_spawn(async move {
            ...
            log::info!("已重建扩展索引，耗时：{:?}", start_time.elapsed());
            index
        })
    }
    ...
    async fn add_extension_to_index(
        ...
        let is_dev = fs
            ...
            .await?
            .with_context(|| format!("缺少扩展目录 {extension_dir:?}"))?
            .is_symlink;
        ...
        if let Ok(mut language_paths) = fs.read_dir(&language_dir).await {
            while let Some(language_path) = language_paths.next().await {
                let language_path = language_path
                    .with_context(|| format!("读取语言目录 {language_dir:?} 中的条目失败"))?;
                ...
                let config = fs.load(&language_config_path).await.with_context(|| {
                    format!("从 {language_config_path:?} 加载语言配置")
                })?;
    ...
    fn prepare_remote_extension(
        ...
        let Some(loaded_extension) = self.extension_index.extensions.get(&extension_id).cloned()
            ...
            return Task::ready(Err(anyhow!("扩展已卸载")));
        };
        let fs = self.fs.clone();
        cx.background_spawn(async move {
