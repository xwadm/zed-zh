...
mod tests {
            ...
            if let AgentThreadEntry::ToolCall(tool_call) = &thread.entries[0] {
                ...
                if let ToolCallContent::ContentBlock(content_block) = &tool_call.content[0] {
                    match content_block {
                        ...
                        ContentBlock::Empty => panic!("预期为 Markdown 内容，但得到空内容"),
                        ContentBlock::ResourceLink { .. } => {
                            panic!("预期为 Markdown 内容，但得到资源链接")
                        }
                        ContentBlock::Image { .. } => {
                            panic!("预期为 Markdown 内容，但得到图片")
                        }
                    }
                } else {
                    panic!("预期为内容块，实际得到：{:?}", tool_call.content[0]);
                }
            } else {
                panic!("预期为工具调用条目，实际得到：{:?}", thread.entries[0]);
            }
        });
    }
    ...
    async fn test_restore_checkpoint_kills_terminal(cx: &mut TestAppContext) {
        ...
        thread.update(cx, |thread, cx| {
            thread
                .handle_session_update(
                    acp::SessionUpdate::ToolCall(
                        acp::ToolCall::new("terminal-tool-1", "正在运行命令")
        ...
        assert!(
            !has_send_task_after,
            "恢复后不应存在发送任务（取消操作应已清除）"
        );

        // 验证条目已被截断（恢复到索引 1 会在 1 处截断，仅保留索引 0）
        let entry_count = thread.read_with(cx, |thread, _| thread.entries.len());
        assert_eq!(
            entry_count, 1,
            "恢复后应仅有 1 条条目（仅保留第一条用户消息）"
        );
    ...
    async fn test_update_last_checkpoint_with_new_message_added(cx: &mut TestAppContext) {
        ...
        let send_future = thread.update(cx, |thread, cx| thread.send_raw("第一条消息", cx));
        ...
        assert!(
            result.is_ok(),
            "即使在 update_last_checkpoint 期间添加新消息，发送也应成功：{:?}",
            result.err()
        );
    }
    ...
    async fn test_follow_up_message_during_generation_does_not_clear_turn(cx: &mut TestAppContext) {
        ...
        assert_eq!(
            ...
            Some(2),
            "发送第二条消息后 running_turn 应设置为回合 2"
        );
        ...
        assert_eq!(
            ...
            Some(2),
            "第一个回合完成时不应清除 running_turn（该值属于回合 2）"
        );
    ...
    async fn test_send_assigns_message_id_without_truncate_support(cx: &mut TestAppContext) {
        ...
        let response = thread
            .update(cx, |thread, cx| thread.send_raw("测试消息", cx))
            .await;

        assert!(response.is_ok(), "发送不应失败：{response:?}");
        thread.read_with(cx, |thread, _| {
    ...
    async fn test_provisional_title_replaced_by_real_title(cx: &mut TestAppContext) {
        ...
        let task = thread.update(cx, |thread, cx| {
            thread.set_title("正在协助解决 Rust 问题".into(), cx)
        });
        task.await.expect("set_title 应成功执行");
        thread.read_with(cx, |thread, _| {
            assert_eq!(
                thread.title().as_ref().map(|s| s.as_str()),
                Some("正在协助解决 Rust 问题")
        ...
        assert_eq!(
            ...
            &[SharedString::from("正在协助解决 Rust 问题")],
            "正式标题应传播到连接"
        );
    }
    ...
    async fn test_session_info_update_replaces_provisional_title_and_emits_event(
        ...
        thread.update(cx, |thread, cx| {
                thread.set_provisional_title("你好，能帮我吗…".into(), cx);
            });
            assert_eq!(
                *title_updated_events.borrow(),
                1,
                "设置临时标题应触发 TitleUpdated 事件"
            );

            let result = thread.update(cx, |thread, cx| {
                thread.handle_session_update(
                    acp::SessionUpdate::SessionInfoUpdate(
                        acp::SessionInfoUpdate::new().title("正在协助解决 Rust 问题"),
                    ),
                    cx,
                )
            });
    ...
    async fn test_usage_update_populates_token_usage_and_cost(cx: &mut TestAppContext) {
        ...
        thread.read_with(cx, |thread, _| {
            let usage = thread.token_usage().expect("应设置令牌使用量");
            assert_eq!(usage.max_tokens, 10000);
            assert_eq!(usage.used_tokens, 5000);

            let cost = thread.cost().expect("应设置费用");
    ...
    async fn test_usage_update_without_cost_preserves_existing_cost(cx: &mut TestAppContext) {
        ...
        thread.read_with(cx, |thread, _| {
            ...
            let cost = thread.cost().expect("应保留费用信息");
    ...
    async fn test_response_usage_does_not_clobber_session_usage(cx: &mut TestAppContext) {
        ...
        thread.read_with(cx, |thread, _| {
            let usage = thread.token_usage().expect("应设置令牌使用数据");
            assert_eq!(usage.max_tokens, 10000, "max_tokens 来自 UsageUpdate");
            assert_eq!(usage.used_tokens, 3000, "used_tokens 来自 UsageUpdate");
            assert_eq!(usage.input_tokens, 200, "input_tokens 来自响应使用数据");
            assert_eq!(
                usage.output_tokens, 300,
                "output_tokens 来自响应使用数据"
            );

            let cost = thread.cost().expect("应设置费用数据");
