...
impl Worktree {
    ...
    pub fn load_file(&self, path: &RelPath, cx: &Context<Worktree>) -> Task<Result<LoadedFile>> {
        match self {
            ...
            Worktree::Remote(_) => {
                Task::ready(Err(anyhow!("远程工作树暂不支持加载文件")))
            }
        }
    }
    ...
    pub fn load_binary_file(
        ...
        match self {
            ...
            Worktree::Remote(_) => {
                Task::ready(Err(anyhow!("远程工作树暂不支持加载二进制文件")))
            }
        }
    }
    ...
    pub fn write_file(
        ...
        match self {
            ...
            Worktree::Remote(_) => {
                Task::ready(Err(anyhow!("远程工作树暂不支持写入文件")))
            }
        }
    }
    ...
    pub async fn handle_expand_entry(
        ...
        task.ok_or_else(|| anyhow::anyhow!("不存在该条目"))?
            .await?;
    ...
    /// 对于可见工作树，返回以工作树名称为第一组件的路径；否则返回绝对路径
...
impl LocalWorktree {
    ...
    fn load_binary_file(
        ...
        cx.background_spawn(async move {
            ...
            let worktree = worktree.upgrade().context("工作树已被释放")?;
            ...
            let file = match entry.await? {
                ...
                None => {
                    let metadata = fs
                        ...
                        .await
                        .with_context(|| {
                            format!("为排除文件加载元数据失败 {abs_path:?}")
                        })?
                        .with_context(|| {
                            format!("排除文件在加载过程中被移除 {abs_path:?}")
                        })?;
                    Arc::new(File {
                        entry_id: None,
                        worktree,
    ...
    pub fn write_file(
        ...
        cx.spawn(async move |this, cx| {
            ...
            let entry = this
                ...
                .await?;
            let worktree = this.upgrade().context("工作树已释放")?;
            if let Some(entry) = entry {
                ...
                let metadata = fs
                    ...
                    .await
                    .with_context(|| {
                        format!("保存排除缓冲区后获取元数据失败 {abs_path:?}")
                    })?
                    .with_context(|| {
                        format!("排除缓冲区在保存过程中被移除 {path:?}")
                    })?;
                Ok(Arc::new(File {
                    worktree,
                    path,
    ...
    pub async fn restore_entry(
        ...
        let Some((fs, worktree_abs_path, path_style)) = this.read_with(cx, |this, _cx| {
            ...
            return Err(anyhow!("本地工作树不能转换为远程工作树"));
        };
        ...
        cx.spawn(async move |this, cx| {
            cx.background_spawn(async move {
                for (source, target) in paths {
                    ...
                    .await
                    .with_context(|| {
                        format!("文件复制失败，从 {source:?} 到 {target:?}")
                    })?;
                }
                anyhow::Ok(())
            })
            ...
            let mut refresh = cx.read_entity(
                &this.upgrade().with_context(|| "工作树已释放")?,
                |this, _| {
                    anyhow::Ok::<postage::barrier::Receiver>(
                        this.as_local()
                            .with_context(|| "工作树不是本地类型")?
            ...
            let this = this.upgrade().with_context(|| "工作树已释放")?;
    ...
    async fn insert_entry(&mut self, mut entry: Entry, fs: &dyn Fs) -> Entry {
        ...
        if entry.is_file() && entry.path.file_name() == Some(&GITIGNORE) {
            ...
            match build_gitignore(&abs_path, fs).await {
                ...
                Err(error) => {
                        log::error!("加载 .gitignore 文件失败 {:?} - {:?}", &entry.path, error);
                    }
            }
        }
    ...
    pub fn check_invariants(&self, git_state: bool) {
        ...
        assert_eq!(
            ...
            self.entries_by_id
                ...
                .into_iter()
                .collect::<Vec<_>>(),
            "entries_by_path 与 entries_by_id 数据不一致"
        );
...
impl BackgroundScannerState {
    ...
    fn populate_dir(
            ...
            .get(&PathKey(parent_path.clone()), ())
        {
            ...
            log::warn!("正在初始化已被删除的目录 {:?}", parent_path);
            return;
        };
    ...
    async fn insert_git_repository(
        ...
        let work_dir_path: Arc<RelPath> = match dot_git_path.parent() {
            Some(parent_dir) => {
                ...
                {
                    log::debug!(
                        "跳过为嵌套的 .git 目录构建 Git 仓库，工作树中的 .git 路径：{dot_git_path:?}"
                    );
                    return;
                };
            ...
            None => {
                ...
                log::debug!(
                    "跳过为工作树本身构建 Git 仓库，工作树中的 .git 路径：{dot_git_path:?}"
                );
                return;
            }
        };
    ...
    async fn insert_git_repository_for_path(
        ...
        watcher
            ...
            .context("添加公共目录到监听器失败")
            .log_err();
        watcher
            .add(&repository_dir_abs_path)
            .context("添加仓库目录到监听器失败")
            .log_err();
...
async fn build_gitignore(abs_path: &Path, fs: &dyn Fs) -> Result<Gitignore> {
    let contents = fs
        .load(abs_path)
        .await
        .with_context(|| format!("加载路径 {} 处的 .gitignore 文件失败", abs_path.display()))?;
...
impl fmt::Debug for Snapshot {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        f.debug_struct("快照")
            .field("ID", &self.id)
            .field("根目录名称", &self.root_name)
            .field("按路径索引的条目", &EntriesByPath(&self.entries_by_path))
            .field("按ID索引的条目", &EntriesById(&self.entries_by_id))
            .finish()
    }
}
...
impl File {
    ...
    pub fn from_proto(
        ...
        let worktree_id = worktree.read(cx).as_remote().context("非远程工作树")?.id();
        ...
        Ok(Self {
            worktree,
            path: RelPath::from_proto(&proto.path).context("文件协议缓冲区中路径无效")?,
            disk_state,
...
impl BackgroundScanner {
    async fn run(&mut self, mut fs_events_rx: Pin<Box<dyn Send + Stream<Item = Vec<PathEvent>>>>) {
        ...
        log::trace!("所属 Git 仓库：{containing_git_repository:?}");
        ...
        loop {
            select_biased! {
                ...
                path_prefix_request = self.path_prefixes_to_scan_rx.recv().fuse() => {
                    ...
                    log::trace!("正在添加路径前缀 {:?}", request.path);
    ...
    async fn process_scan_request(&self, mut request: ScanRequest, scanning: bool) -> bool {
        log::debug!("重新扫描路径 {:?}", request.relative_paths);
        ...
        let root_canonical_path = match &root_canonical_path {
            ...
            Err(err) => {
                log::error!("规范化根路径 {root_path:?} 失败：{err:#}");
                return true;
            }
        };
...

        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                if let Some((dot_git_abs_path, path_in_git_dir)) = dot_git_paths {
                    ...
                    if skip {
                        log::debug!(
                            "忽略事件 {abs_path:?}，原因：该路径位于 .git 目录中，属于跳过的文件或目录"
                        );
...

        {
            ...
            for (ix, event) in events.iter().enumerate() {
                ...
                if !parent_dir_is_loaded {
                    log::debug!("忽略事件 {relative_path:?}，路径位于未加载的目录中");
                    skip_ix(&mut ranges_to_drop, ix);
                    continue;
                }
                            ...
                            select_biased! {
                                ...
                                job = scan_jobs_rx.recv().fuse() => {
                                    ...
                                    if let Err(err) = self.scan_dir(&job).await
                                        && job.path.is_empty() {
                                            log::error!("扫描目录 {:?} 时出错：{}", job.abs_path, err);
                                        }
                                }
                            }
                        }
    ...
    async fn scan_dir(&self, job: &ScanJob) -> Result<()> {
        ...
        for child_abs_path in child_paths {
            ...
            if self.settings.is_path_excluded(&child_path) {
                log::debug!("跳过已排除的子条目 {child_path:?}");
                self.state
                    .lock()
                    .await
            ...
            if job.is_external {
                ...
                let canonical_path = match self.fs.canonicalize(&child_abs_path).await {
                    Ok(path) => path,
                    Err(err) => {
                        log::error!("读取符号链接目标 {child_abs_path:?} 时出错：{err:#}");
                        continue;
                    }
                };
                ...
                let root_canonical_path = match &root_canonical_path {
                    ...
                    None => match self.fs.canonicalize(&root_abs_path).await {
                        ...
                        Err(err) => {
                            log::error!("规范化根路径 {root_abs_path:?} 时出错：{err:?}");
                            continue;
                        }
                    },
                };
...

            {
                ...
                if self.settings.is_path_hidden(&relative_path) {
                    log::debug!("检测到隐藏文件：{relative_path:?}");
    ...
    async fn reload_entries_for_paths(
        ...
        for (path, metadata) in relative_paths.iter().zip(metadata.into_iter()) {
            ...
            match metadata {
                ...
                Err(err) => {
                    log::error!("处理事件时读取文件 {abs_path:?} 出错：{err:#}");
                }
            }
        }
...
async fn discover_ancestor_git_repo(
    ...
    for (index, ancestor) in root_abs_path.as_path().ancestors().enumerate() {
        ...
        log::trace!("正在检查上级目录：{ancestor_dot_git:?}");
            ...
            if index != 0 {
                ...
                if let Some(ancestor_dot_git) = fs.canonicalize(&ancestor_dot_git).await.log_err() {
                    let location_in_repo = root_abs_path
                        ...
                        .unwrap()
                        .into();
                    log::info!("正在为该工作树插入父 Git 仓库：{location_in_repo:?}");
        ...
        match path.cmp(&&event_root.path) {
            Ordering::Less => {
                    merged_event_roots.push(EventRoot {
                        path: (*changed_paths.next().expect("已预查的变更路径")).clone(),
                        was_rescanned: false,
                    });
                }
                Ordering::Equal => {
                    merged_event_roots.push((*event_roots.next().expect("已预查的事件根路径")).clone());
                    changed_paths.next();
                }
                Ordering::Greater => {
                    merged_event_roots.push((*event_roots.next().expect("已预查的事件根路径")).clone());
...
impl TryFrom<(&CharBag, &PathMatcher, proto::Entry)> for Entry {
    ...
    fn try_from(
        ...
        let path =
            RelPath::from_proto(&entry.path).context("协议消息中存在无效的相对路径")?;
        let char_bag = char_bag_for_path(*root_char_bag, &path);
...
/// 解析 gitfile 内容，提取 git 目录路径
fn parse_gitfile(content: &str) -> anyhow::Result<&Path> {
    // 移除 "gitdir:" 前缀并获取后续路径
    let path = content
        .strip_prefix("gitdir:")
        .with_context(|| format!("解析 gitfile 内容失败 {content:?}"))?;
...
async fn decode_file_text(
    ...
    loop {
        ...
        let n = file
            .read(&mut buf)
            .with_context(|| format!("读取文件 {abs_path:?} 的字节数据失败"))?;
        if n == 0 {
            reached_eof = true;
            break;
        }
    ...
    if !reached_eof {
        ...
        loop {
            let n = file
                .read(&mut buf)
                .with_context(|| format!("读取文件 {abs_path:?} 的剩余字节失败"))?;
            if n == 0 {
                break;
            }
...
mod tests {
    ...
    fn test_pcm16_wav_detected_as_binary() {
        ...
        assert_eq!(
            result,
            ByteContent::Binary,
            "通过 RIFF 头部应将16位PCM WAV识别为二进制文件"
        );
    }
    ...
    fn test_le16_binary_not_misdetected_as_utf16le() {
        ...
        assert_eq!(
            result,
            ByteContent::Binary,
            "包含控制字符的小端序 16 位二进制文件应被识别为二进制文件"
        );
    }
    ...
    fn test_be16_binary_not_misdetected_as_utf16be() {
        ...
        assert_eq!(
            result,
            ByteContent::Binary,
            "包含控制字符的大端序 16 位二进制文件应被识别为二进制文件"
        );
    }

    #[test]
    fn test_utf16le_text_detected_as_utf16le() {
        // 定义用于测试的 UTF-16 编码测试字符串
