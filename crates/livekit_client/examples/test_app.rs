...
actions!(livekit_client, [退出]);

fn main() {
    SimpleLogger::init(LevelFilter::Info, Default::default()).expect("日志初始化失败");

    gpui_platform::application().run(|cx| {
        #[cfg(any(test, feature = "test-support"))]
        println!("使用测试版 LiveKit");

        #[cfg(not(any(test, feature = "test-support")))]
        println!("使用正式版 LiveKit");
        ...
        cx.set_menus([Menu::new("Zed").items([MenuItem::action("退出", 退出)])]);
    ...
    async fn new(
        ...
        let (room, mut events) =
            Room::connect(url.clone(), token, cx)
                ...
                .unwrap_or_else(|err| {
                    eprintln!(
                        "连接 {url} 失败：{err}。\n请尝试执行 `foreman start` 启动 LiveKit 服务器"
                    );
                    ...
                    .children(self.remote_participants.iter().map(|(identity, state)| {
                        div()
                            ...
                            .child(SharedString::from(if state.speaking {
                                format!("{}（正在说话）", &identity.0)
                            } else if state.muted {
                                format!("{}（已静音）", &identity.0)
                            } else {
                                identity.0.clone()
                            }))
