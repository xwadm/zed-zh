...
// 可视化测试模块（需要 macOS 和 visual-tests 特性）
...
pub fn init(cx: &mut App) {
    ...
    cx.observe_flag::<PanicFeatureFlag, _>({
        ...
        move |flag, cx| {
            ...
            added = true;
            cx.on_action(|_: &TestPanic, _| panic!("运行了 TestPanic 动作"))
...
/// 绑定窗口关闭时的行为：根据设置决定是否在最后一个窗口关闭时退出应用。
