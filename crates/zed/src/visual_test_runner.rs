...
//! 可视化测试运行器
//!
//! 该二进制程序用于为 Zed 的 UI 运行视觉回归测试。它会对真实的 Zed 窗口进行截图，
//! 并与基准图像进行对比。
//!
//! **注意：此工具仅支持 macOS**，因为它使用的 `VisualTestAppContext`
//! 依赖 macOS Metal 渲染器来实现精准的截图捕获。
//!
//! ## 工作原理
//!
//! 该工具使用 `VisualTestAppContext`，整合了以下能力：
//! - 真实的 Metal/合成器渲染，保证截图精准
//! - 通过 TestDispatcher 实现确定性任务调度
//! - 通过 `advance_clock` 控制时间，用于测试基于时间的行为
//!
//! 该方案的特点：
//! - 不需要屏幕录制权限
//! - 不需要窗口在屏幕上可见
//! - 捕获原始 GPUI 输出，不含系统窗口边框
//! - 完全确定性——提示框、动画等均可稳定测试
//!
//! ## 使用方法
//!
...
//! ## 环境变量
//!
//!   UPDATE_BASELINE - 设置为更新基准图像而非进行对比
//!   VISUAL_TEST_OUTPUT_DIR - 保存测试输出的目录（默认：target/visual_tests）
