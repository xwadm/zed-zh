...
pub fn init(startup_time: Instant, cx: &mut App) {
    ...
    cx.observe_global::<SettingsStore>(|cx| {
        ...
        // `set_enabled` 报告值是否实际发生了变化，因此跳过常见无操作路径上的
        // 过滤器更新和窗口清理 —— 设置观察者会在每次设置更改时触发。
...
impl ProfilerWindow {
    ...
    fn poll_timings(&mut self, cx: &App) {
        ...
        match self.source {
            ...
            ProfileSource::RemoteForeground | ProfileSource::RemoteAllThreads => {
                // 远程计时数据通过 apply_remote_response 异步到达。
            }
        }
...
/// 将 K 个已排序的 `Vec<SerializedTaskTiming>` 合并为一个已排序的向量。
/// 每个输入向量必须已按 `start` 排序。
