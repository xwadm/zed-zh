    ...
    cx.observe_new(|workspace: &mut Workspace, _, _| {
        ...
        workspace.register_action(|workspace, _: &CopyRoomId, window, cx| {
            ...
            if let Some(room) = ActiveCall::global(cx).read(cx).room() {
                ...
                workspace.show_error(&"当前无活跃通话，请先加入一个通话。", cx);
            }
        });
...
impl CollabPanel {
                    ...
                    window.handler_for(&this, move |_, window, cx| {
                        ActiveCall::global(cx)
                            ...
                            .detach_and_prompt_err("授予写入权限失败", window, cx, |e, _, _| {
                                match e.error_code() {
                                    ErrorCode::NeedsCla => Some("该用户尚未签署 CLA，签署地址：https://zed.dev/cla。".into()),
                                    _ => None,
                                }
                            })
                    }),
            ...
            .detach_and_prompt_err("设置频道可见性失败", window, cx, |e, _, _| match e.error_code() {
                    ErrorCode::BadPublicNesting =>
                        if e.error_tag("direction") == Some("parent") {
                            ...
                            Some("要将频道设为私密，其所有子频道必须是私密的。".to_string())
                        },
                    _ => None
                });
    }
    ...
    fn render_signed_out(&mut self, cx: &mut Context<Self>) -> Div {
        let collab_blurb = "通过实时协作编辑、语音通话、共享笔记等功能，与团队高效协同工作。";
    ...
    fn render_header(
        ...
        let text = match section {
            ...
            Section::FavoriteChannels => SharedString::from("收藏频道"),
            Section::ContactRequests => SharedString::from("联系人请求"),
        ...
        match &entry.notification {
            Notification::ContactRequest { sender_id } => {
                ...
                Some((
                    ...
                    format!("{} 希望添加你为联系人", requester.github_login),
                ))
            }
            ...
            Notification::ChannelInvitation {
                ...
                Some((
                    ...
                    format!(
                        "{} 邀请你加入 #{} 频道",
                        inviter.github_login, channel_name
                    ),
                ))
            }
        }
    ...
    pub fn entries_as_strings(&self) -> Vec<String> {
        ...
        for (index, entry) in self.entries.iter().enumerate() {
            ...
            match entry {
                ...
                ListEntry::ParticipantScreen { .. } => {
                    string_entries.push(format!("    (屏幕共享){selected_marker}"));
                }
                ListEntry::IncomingRequest(user) => {
                    string_entries.push(format!(
                        "  (待处理) {}{selected_marker}",
