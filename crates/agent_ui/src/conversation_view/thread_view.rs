...
impl ThreadView {
    ...
    ThreadError::PaymentRequired => (
        "payment_required",
        None,
        "你已达到免费使用限额。升级到 Zed Pro 以获取更多提示次数。".into(),
    ),
    ThreadError::Refusal => {
        let model_or_agent_name = self.current_model_name(cx);
        let message = format!(
            "{} 拒绝响应此提示。当模型认为提示违反其内容政策或安全准则时可能会发生这种情况，因此重新表述提示有时可以解决问题。",
            model_or_agent_name
        );
    ...
    ThreadError::RateLimitExceeded { provider } => (
        "rate_limit_exceeded",
        None,
        format!("已达到 {} 的调用频率限制。", provider).into(),
    ),
    ThreadError::ServerOverloaded { provider } => (
        "server_overloaded",
        None,
        format!("{} 的服务器暂时不可用。", provider).into(),
    ),
    ThreadError::PromptTooLarge => (
        "prompt_too_large",
        None,
        "上下文内容超出模型上下文窗口的大小限制。".into(),
    ),
    ThreadError::NoApiKey { provider } => (
        "no_api_key",
        None,
        format!("未为 {} 配置 API 密钥。", provider).into(),
    ),
    ThreadError::StreamError { provider } => (
        "stream_error",
        None,
        format!("与 {} 的 API 连接中断。", provider).into(),
    ),
    ThreadError::InvalidApiKey { provider } => (
        "invalid_api_key",
        None,
        format!("{} 的 API 密钥无效或已过期。", provider).into(),
    ),
    ThreadError::PermissionDenied { provider } => (
        "permission_denied",
        None,
        format!("{} 的 API 因权限不足拒绝了该请求。", provider)
            .into(),
    ),
    ...
    ThreadError::NoModelSelected => {
        ("no_model_selected", None, "未选择任何模型。".into())
    }
    ThreadError::ApiError { provider } => (
        "api_error",
        None,
        format!("{} 的 API 返回了意外错误。", provider).into(),
    ),
    ThreadError::Other {
        acp_error_code,
        message,
    ...
    fn toggle_following(&mut self, window: &mut Window, cx: &mut Context<Self>) {
        ...
        telemetry::event!("已选择关注代理", following = !following);
    }
    ...
    pub fn render_thread_retry_status_callout(&self) -> Option<Callout> {
        ...
        let retry_message = if state.max_attempts == 1 {
            if next_attempt_in_secs == 1 {
                "正在重试。1秒后进行下一次尝试。".to_string()
            } else {
                format!("正在重试。{next_attempt_in_secs}秒后进行下一次尝试。")
            }
        } else if next_attempt_in_secs == 1 {
            format!(
                "正在重试。1秒后进行下一次尝试（第 {}/{} 次尝试）。",
                state.attempt, state.max_attempts,
            )
        } else {
            format!(
                "正在重试。{next_attempt_in_secs}秒后进行下一次尝试（第 {}/{} 次尝试）。",
                state.attempt, state.max_attempts,
            )
        };
    ...
    fn render_effort_selector(
        ...
        let label = selected
            ...
            .map_or("选择努力程度".into(), |effort| effort.name);
    ...
    fn render_send_button(&self, cx: &mut Context<Self>) -> AnyElement {
        ...
        if self.is_loading_contents {
            div()
                ...
                .px_1()
                .tooltip(Tooltip::text("正在加载添加上下文…"))
            ...
            IconButton::new("send-message", send_icon)
                ...
                .tooltip(move |_window, cx| {
                    if is_editor_empty && !is_generating {
                        Tooltip::for_action("输入内容发送", &Chat, cx)
                    ...
                    ContextMenuEntry::new("文件与目录")
                        ...
                        .handler({
                            ...
                            move |window, cx| {
                                ...
                                message_editor.update(cx, |editor, cx| {
                                    editor.insert_context_type("文件", window, cx);
                                });
                            }
                        }),
                )
                    ...
                    ContextMenuEntry::new("符号")
                        ...
                        .handler({
                            ...
                            move |window, cx| {
                                message_editor.focus_handle(cx).focus(window, cx);
                                message_editor.update(cx, |editor, cx| {
                                    editor.insert_context_type("符号", window, cx);
                                });
                            }
                        }),
                )
                    ...
                    ContextMenuEntry::new("对话线程")
                        ...
                        .handler({
                            ...
                            move |window, cx| {
                                message_editor.focus_handle(cx).focus(window, cx);
                                message_editor.update(cx, |editor, cx| {
                                    editor.insert_context_type("对话线程", window, cx);
                                });
                            }
                        }),
                )
                    ...
                    ContextMenuEntry::new("规则")
                        ...
                        .handler({
                            ...
                            move |window, cx| {
                                message_editor.focus_handle(cx).focus(window, cx);
                                message_editor.update(cx, |editor, cx| {
                                    editor.insert_context_type("规则", window, cx);
                                });
                            }
                        }),
                )
...
impl ThreadView {
                            ...
                            h_flex()
                                ...
                                .child(
                                    Button::new("restore-checkpoint", "恢复检查点")
                                        ...
                                        .label_size(LabelSize::XSmall)
                                        .color(Color::Muted)
                                        .tooltip(Tooltip::text("将项目中的所有文件恢复到对话此节点时的内容状态。"))
                                        .on_click(cx.listener(move |this, _, _window, cx| {
            ...
            v_flex()
                ...
                .child(
                    h_flex()
                        ...
                        .child(Divider::horizontal())
                        .tooltip(Tooltip::text("此线下的所有内容均为该子助手发送给主助手的输出信息。")),
                )
                .child(primary)
                .into_any_element()
        } else {
    ...
    fn render_thread_controls(
        ...
        let open_as_markdown = IconButton::new("open-as-markdown", IconName::FileMarkdown)
            ...
            .tooltip(Tooltip::text("以 Markdown 格式打开对话线程"))
            .on_click(cx.listener(move |this, _, window, cx| {
                if let Some(workspace) = this.workspace.upgrade() {
        ...
        let scroll_to_recent_user_prompt =
            IconButton::new("scroll_to_recent_user_prompt", IconName::ForwardArrow)
                ...
                .icon_size(IconSize::Small)
                .icon_color(Color::Ignored)
                .tooltip(Tooltip::text("滚动至最新的用户提示"))
                .on_click(cx.listener(move |this, _, _, cx| {
        ...
        let last_turn_tokens_label = last_turn_clock
            ...
            .then(|| {
                self.turn_fields
                    ...
                    .map(|tokens| {
                        Label::new(format!("{} 个令牌",crate::humanize_token_count(tokens)))
                            .size(LabelSize::Small)
                            .color(Color::Muted)
                    })
            })
                    ...
                    .child(
                        IconButton::new("feedback-thumbs-up", IconName::ThumbsUp)
                            ...
                            .tooltip(move |window, cx| match feedback {
                                ...
                                _ => {
                                    Tooltip::with_meta("有用的回复", None, tooltip_meta(), cx)
                                }
                            })
                            .on_click(cx.listener(move |this, _, window, cx| {
            ...
            .menu(move |window, cx| {
                ...
                ContextMenu::build(window, cx, move |menu, _, cx| {
                    ...
                    let copy_this_agent_response =
                        ContextMenuEntry::new("复制此助手回复").handler({
                            let entity = entity.clone();
                            move |_, cx| {
                    ...
                    let open_thread_as_markdown = ContextMenuEntry::new("以 Markdown 格式打开对话线程")
                        .handler({
                            let entity = entity.clone();
                            let workspace = workspace.clone();
                            move |window, cx| {
            ...
            .when(truncated_output, |header| {
                let tooltip = if let Some(output) = output {
                    if output_line_count + 10 > terminal::MAX_SCROLL_HISTORY_LINES {
                       format!("输出内容超出终端最大行数限制，已被截断，模型已接收前 {} 内容。", format_file_size(output.content.len() as u64, true))
                    } else {
                        format!(
                            "输出内容长度为 {}，为避免产生意外的令牌消耗，仅向助手返回了 {} 内容。",
...
;
    ...
    fn render_permission_granularity_dropdown_with_patterns(
        ...
        let pattern_options: Vec<(usize, SharedString)> = patterns
            ...
            .map(|(i, cp)| {
                (
                    i,
                    SharedString::from(format!("对 `{}` 命令始终允许", cp.display_name)),
                )
            })
            .collect();
    ...
    fn render_markdown_output(
        ...
        let button_id = SharedString::from(format!("工具输出-{:?}", tool_call_id));
                ...
                this.child(
                    h_flex()
                        ...
                        .when_some(location, |this, _loc| {
                            this.child(
                                Button::new(("go-to-file", entry_ix), "前往文件")
                                    .label_size(LabelSize::Small)
                                    .on_click(cx.listener(move |this, _, window, cx| {
                                        this.open_tool_call_location(entry_ix, 0, window, cx);
                                    })),
    ...
    fn render_subagent_card(
        ...
        let card_header_id = format!("子代理头部-{}", entry_ix);
        let status_icon = format!("状态图标-{}", entry_ix);
        let diff_stat_id = format!("子代理差异-{}", entry_ix);
    ...
    pub(crate) fn render_thread_error(
        ...
        let content = match self.thread_error.as_ref()? {
            ...
            ThreadError::ServerOverloaded { provider } => self.render_error_callout(
                "服务提供商不可用",
                format!(
                    "{provider} 的服务器暂时无法访问。Zed 将自动重试。\
                    如果问题仍然存在，请查看服务提供商的状态页面。"
                )
                .into(),
                true,
                true,
            ...
            ThreadError::NoApiKey { provider } => self.render_error_callout(
                "缺少 API 密钥",
                format!(
                    "未为 {provider} 配置 API 密钥。\
                    请通过代理面板设置添加你的密钥以继续使用。"
                )
                .into(),
                false,
                true,
            ...
            ThreadError::StreamError { provider } => self.render_error_callout(
                "连接中断",
                format!(
                    "与 {provider} API 的连接已中断。Zed 将自动重试。\
                    如果问题仍然存在，请检查你的网络连接。"
                )
                .into(),
                true,
            ...
            ThreadError::InvalidApiKey { provider } => self.render_error_callout(
                "API 密钥无效",
                format!(
                    "{provider} 的 API 密钥无效或已过期。\
                    请通过代理面板设置更新你的密钥以继续使用。"
                )
                .into(),
                false,
                false,
            ...
            ThreadError::PermissionDenied { provider } => self.render_error_callout(
                "权限被拒绝",
                format!(
                    "{provider} API 因权限不足拒绝了该请求。\
                    请检查你的 API 密钥是否有权访问此模型。"
                )
                .into(),
                false,
                false,
            ...
            ThreadError::ApiError { provider } => self.render_error_callout(
                "API 错误",
                format!(
                    "{provider} API 返回了意外错误。\
                    如果问题仍然存在，请尝试切换模型或重启 Zed。"
                )
                .into(),
                true,
                true,
    ...
    const ERROR_MESSAGE: &str =
        "你已达到免费使用限额。升级到 Zed Pro 以获取更多提示词额度。";
    ...
    const MESSAGE: &str = "本次对话过长，超出了模型的上下文窗口限制。\
        请开启新对话或移除部分附加文件后继续。";
...
fn current_model_name(&self, cx: &App) -> SharedString {
    // 对于原生代理（Zed 代理），使用具体模型名称（例如 "Claude 3.5 Sonnet"）
    // 对于 ACP 代理，使用代理名称（例如 "Claude 代理"、"Gemini CLI"）
    // 这样能更清晰地显示是哪个服务拒绝了请求
    if self.as_native_connection(cx).is_some() {
        ...
        // ACP 代理 - 使用代理名称（例如 "Claude 代理"、"Gemini CLI"）
...
fn render_resume_notice(_cx: &Context<Self>) -> AnyElement {
    let description = "该代理不支持查看历史消息，但你的会话仍会从上次中断的位置继续。";
...
fn render_codex_windows_warning(&self, cx: &mut Context<Self>) -> Callout {
    Callout::new()
        ...
        .description("为获得最佳性能，请在 Windows 子系统（WSL2）中运行 Codex")
        .actions_slot(
...
fn render_external_source_prompt_warning(&self, cx: &mut Context<Self>) -> Callout {
    Callout::new()
        ...
        .severity(Severity::Warning)
        .title("发送前请检查")
        .description("该提示词由外部链接预填充，发送前请仔细确认内容。")
        .dismiss_action(
...
fn render_token_limit_callout(&self, cx: &mut Context<Self>) -> Option<Callout> {
    ...
    let description = "如需继续，请从摘要创建新对话。";
        ...
        Callout::new()
            ...
            .actions_slot(
                h_flex().gap_0p5().child(
                    Button::new("start-new-thread", "创建新对话")
                        .label_size(LabelSize::Small)
                        .on_click(cx.listener(|this, _, window, cx| {
