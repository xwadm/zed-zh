...
impl AgentConfiguration {
    ...
    fn render_provider_configuration_section(
        ...
        let popover_menu = PopoverMenu::new("add-provider-popover")
            .trigger(
                Button::new("add-provider", "添加服务提供方")
                    .style(ButtonStyle::Outlined)
                    .start_icon(
            ...
            .menu({
                ...
                move |window, cx| {
                    Some(ContextMenu::build(window, cx, |menu, _window, _cx| {
                        menu.header("兼容的 API").entry("OpenAI", None, {
        ...
        v_flex()
            ...
            .child(self.render_section_title(
                "大语言模型服务提供方",
            "至少添加一个提供方，才能结合 Zed 原生智能体使用 AI 功能。",
    ...
    fn render_zed_plan_info(&self, plan: Option<Plan>, cx: &mut Context<Self>) -> impl IntoElement {
        if let Some(plan) = plan {
            ...
            let (plan_name, label_color, bg_color) = match plan {
                ...
                Plan::ZedProTrial => ("Pro 试用版", Color::Accent, pro_chip_bg),
                Plan::ZedPro => ("Pro 版", Color::Accent, pro_chip_bg),
        ...
        let display_name = if provided_by_extension {
            resolve_extension_for_context_server(&context_server_id, cx)
                .map(|(_, manifest)| {
                    ...
                    let stripped = name
                        .strip_suffix(" MCP 服务器")
                        .or_else(|| name.strip_suffix(" MCP"))
                        .or_else(|| name.strip_suffix(" 上下文服务器"))
                        .unwrap_or(name);
                ...
                feedback_base_container()
                    .child(
                        h_flex()
                            ...
                            .child(
                                Label::new("进行身份验证以连接此服务器")
                                    .color(Color::Muted)
                                    .size(LabelSize::Small),
                            ),
                    )
                    .child(
                        Button::new("error-logout-server", "进行身份验证")
                            .style(ButtonStyle::Outlined)
                            .label_size(LabelSize::Small)
                            .on_click({
                                let context_server_id = context_server_id.clone();
    ...
    fn render_agent_server(
        ...
        let uninstall_button = match source {
            ExternalAgentSource::Extension => Some(
                IconButton::new(
                    SharedString::from(format!("卸载-{}", id)),
                    IconName::Trash,
                )
                .icon_color(Color::Muted)
                .icon_size(IconSize::Small)
                .tooltip(Tooltip::text("卸载智能体扩展"))
