...
mod tests {
    ...
    fn test_fuzzy_match_positions_returns_byte_indices() {
        ...
        let text = "🔥abc";
        let positions = fuzzy_match_positions("ab", text).expect("应该匹配");
        ...
        for &pos in &positions {
            assert!(
                ...
                "位置 {pos} 不是 {text:?} 中有效的 UTF-8 字符边界"
            );
        }
    }
    ...
    fn test_fuzzy_match_positions_ascii_still_works() {
        let positions = fuzzy_match_positions("he", "hello").expect("应该匹配");
    ...
    fn test_fuzzy_match_positions_multi_byte_interior() {
        ...
        let text = "café";
        let positions = fuzzy_match_positions("fé", text).expect("应该匹配");
