...
impl Render for ProfileSelector {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        if !self.provider.profiles_supported(cx) {
            return Button::new("tools-not-supported-button", "暂不支持工具")
                .disabled(true)
                .label_size(LabelSize::Small)
                .color(Color::Muted)
                .tooltip(Tooltip::text("该模型不支持工具功能。"))
                .into_any_element();
        }
...
impl ProfilePickerDelegate {
    ...
    fn documentation(candidate: &ProfileCandidate) -> Option<&'static str> {
        match candidate.id.as_str() {
            builtin_profiles::WRITE => Some("获取写作辅助，支持各类内容创作。"),
            builtin_profiles::ASK => Some("围绕代码库进行问答交流。"),
            builtin_profiles::MINIMAL => Some("无工具模式，自由聊天。"),
            _ => None,
        }
    }
...
mod tests {
    ...
    fn entries_include_custom_profiles(_cx: &mut TestAppContext) {
        ...
        ProfileCandidate {
            ...
            name: SharedString::from("写作"),
            is_builtin: true,
        },
        ProfileCandidate {
            id: AgentProfileId("my-custom".into()),
            name: SharedString::from("我的自定义配置"),
            is_builtin: false,
        },
    ];
        ...
        assert!(entries.iter().any(|entry| matches!(
                    entry,
                    ProfilePickerEntry::Header(label) if label.as_ref() == "自定义配置文件"
                )));
    }
