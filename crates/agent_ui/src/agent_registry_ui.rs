...
enum RegistryInstallStatus {
    NotInstalled,     // 未安装
    InstalledRegistry,// 已安装（注册表）
    InstalledCustom,  // 已安装（自定义）
    InstalledExtension,// 已安装（扩展）
}
...
impl AgentRegistryPage {
    ...
    pub fn new(
        ...
        cx.new(|cx| {
            ...
            let query_editor = cx.new(|cx| {
                ...
                input.set_placeholder_text("搜索代理...", window, cx);
                input
            });
    ...
    fn render_registry_agent(
        ...
        let website_button = agent.website().map(|website| {
            ...
            .icon_size(IconSize::Small)
            .tooltip(move |_, cx| {
                Tooltip::with_meta("访问代理官网", None, website.clone(), cx)
            })
            .on_click(move |_, _, cx| {
