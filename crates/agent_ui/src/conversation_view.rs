...
impl ConversationView {
    ...
    fn initial_state(
        ...
        let side = match AgentSettings::get_global(cx).sidebar_side() {
            SidebarSide::Left => "左侧",
            SidebarSide::Right => "右侧",
        };
        let thread_location = match AgentSettings::get_global(cx).new_thread_location {
            NewThreadLocation::LocalProject => "当前工作树",
            NewThreadLocation::NewWorktree => "新工作树",
        };
        ...
        match &self.server_state {
            ...
            ServerState::LoadError { error, .. } => match error {
                ...
                LoadError::FailedToInstall(_) => {
                    format!("{} 安装失败", self.agent.agent_id()).into()
                }
        ...
        self.auth_task = Some(cx.spawn_in(window, {
            async move |this, cx| {
                ...
                match &result {
                    ...
                    Err(_) => {
                        telemetry::event!("代理认证失败", agent = agent_telemetry_id,)
                    }
                }
    ...
    fn spawn_external_agent_login(
        ...
        window.spawn(cx, async move |cx| {
            ...
            task.shell = task::Shell::WithArguments {
                program: task.command.take().expect("应设置登录命令"),
            ...
            if success_patterns.is_empty() {
                ...
                None => Err(anyhow!("登录命令异常终止，无退出状态")),
            }
            } else {
    ...
    fn render_auth_required_state(
        ...
        if pending_auth_method.is_some() {
            return Callout::new()
                .icon(IconName::Info)
                .title(format!("正在认证 {}…", agent_display_name))
                .actions_slot(
        ...
        Callout::new()
            .icon(IconName::Info)
            .title(format!("认证 {}", agent_display_name))
    ...
    fn emit_load_error_telemetry(&self, error: &LoadError) {
        let error_kind = match error {
            LoadError::Unsupported { .. } => "不支持",
            LoadError::FailedToInstall(_) => "安装失败",
            LoadError::Exited { .. } => "已退出",
            LoadError::Other(_) => "其他",
        };
    ...
    fn render_load_error(
        ...
        let (title, message, action_slot): (_, SharedString, _) = match e {
            ...
            LoadError::Exited { status } => (
                "启动失败",
                format!("服务端异常退出，状态码：{status}").into(),
                None,
            ),
            LoadError::Other(msg) => (
                "启动失败",
    ...
    fn render_unsupported(
        ...
        let (heading_label, description_label) = (
            format!("升级 {} 以适配 Zed 使用", self.agent.agent_id()),
            if version.is_empty() {
                format!(
                    "当前使用的 {} 未返回有效的 --version 信息",
                    path,
                )
            } else {
                format!(
                    "当前使用的 {} 版本为 {}（需至少 {} 版本）",
                    path, version, minimum_version
                )
            },
        );
                    ...
                    move |this, _, event, window, cx| match event {
                        AgentNotificationEvent::Accepted => {
                            let Some(handle) = window.window_handle().downcast::<MultiWorkspace>()
                                ...
                                log::error!("根视图必须为多工作区（MultiWorkspace）类型");
                                return;
                            };
                            cx.activate(true);
...
fn placeholder_text(agent_name: &str, has_commands: bool) -> String {
    if agent_name == agent::ZED_AGENT_ID.as_ref() {
        format!("向 {} 发送消息 — @ 可添加上下文", agent_name)
...
pub(crate) mod tests {
    ...
    async fn test_external_source_prompt_requires_manual_send(cx: &mut TestAppContext) {
        ...
        let Some(prompt) = crate::ExternalSourcePrompt::new("为我编写一个脚本") else {
            panic!("预期外部来源的提示词可成功完成净化处理");
        };
        ...
        active_thread(&conversation_view, cx).read_with(cx, |view, cx| {
            ...
            assert_eq!(view.message_editor.read(cx).text(cx), "为我编写一个脚本");
        });
    }
    ...
    async fn test_acp_server_exit_transitions_conversation_to_load_error_without_panic(
        ...
        conversation_view.read_with(cx, |view, _cx| {
            assert!(
                ...
                "ACP 线程退出时，对话应切换至 LoadError 状态"
            );
        });
        assert_eq!(
            close_session_count.load(std::sync::atomic::Ordering::SeqCst),
            1,
            "线程退出后，ConversationView 应关闭 ACP 会话"
        );
    }
    ...
    async fn test_restored_threads_keep_available_commands(cx: &mut TestAppContext) {
        ...
        active_thread(&conversation_view, cx).read_with(cx, |view, _cx| {
            ...
            assert_eq!(available_commands[0].description.as_str(), "获取帮助");
        });

        assert_eq!(
            placeholder,
            Some("向 Test 发送消息 — @ 可添加上下文，/ 可调用命令".to_string())
        );
    ...
    async fn test_refusal_handling(cx: &mut TestAppContext) {
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("执行有害操作", window, cx);
        });
        ...
        conversation_view.read_with(cx, |view, cx| {
            ...
            match &view.server_state {
                ServerState::LoadError {
                    ...
                    assert!(
                        msg.contains("无效的 gzip 头部"),
                        "错误提示应包含底层解压错误信息，实际获取：{msg}"
                    );
                }
                other => panic!(
                    "预期为 LoadError::Other，实际得到：{}",
                    match other {
                        ServerState::Loading { .. } => "加载中（卡住！）",
                        ServerState::LoadError { .. } => "加载错误（错误类型）",
    ...
    async fn test_reset_preserves_session_id_after_load_error(cx: &mut TestAppContext) {
        ...
        let resume_session_id = acp::SessionId::new("持久化会话");
        let stored_title: SharedString = "持久化对话".into();
        ...
        let connected = view
            .as_connected()
            .expect("不稳定服务器恢复后应处于已连接状态");
        let active_id = connected
            .active_id
            .as_ref()
            .expect("已连接状态应包含有效会话ID");
        assert_eq!(
            active_id, &resume_session_id,
            "reset() 必须恢复原始会话ID，而非调用 new_session()"
        );
    ...
    async fn test_auth_required_on_initial_connect(cx: &mut TestAppContext) {
        ...
        conversation_view.read_with(cx, |view, _cx| {
            assert!(
                ...
                "未认证且无会话时，active_thread() 应返回 None"
            );
        });
        ...
        let connected = view
            .as_connected()
            .expect("认证完成后仍应处于已连接状态");
        assert!(connected.auth_state.is_ok(), "认证状态应为成功");
        assert!(
    ...
    async fn test_notification_for_tool_authorization(cx: &mut TestAppContext) {
        ...
        let tool_call = acp::ToolCall::new(tool_call_id.clone(), "标签")
    ...
    async fn test_notification_respects_never_setting(cx: &mut TestAppContext) {
        ...
        assert!(
            !cx.windows()
                .iter()
                .any(|window| window.downcast::<AgentNotification>().is_some()),
            "当 notify_when_agent_waiting 设置为 Never 时，预期不会显示通知"
        );
    }
        ...
        fn new_session(
            ...
            if !*self.authenticated.lock() {
                return Task::ready(Err(acp_thread::AuthRequired::new()
                    .with_description("请登录后继续".to_string())
                    .into()));
            }
        ...
        fn authenticate(
            ...
            if &method_id == self.auth_method.id() {
                *self.authenticated.lock() = true;
                ...
                Task::ready(Err(anyhow::anyhow!("未知的认证方式")))
            }
        }
    ...
    async fn test_rewind_views(cx: &mut TestAppContext) {
        ...
        acp::ToolCall::new("tool1", "编辑文件 1")
            .kind(acp::ToolKind::Edit)
            .status(acp::ToolCallStatus::Completed)
            .content(vec![acp::ToolCallContent::Diff(
                acp::Diff::new("/project/test1.txt", "新内容 1").old_text("旧内容 1"),
            )]),
    )]);

        thread
            .update(cx, |thread, cx| thread.send_raw("给我一个差异对比", cx))
            .await
            .unwrap();
        cx.run_until_parked();
        ...
        // Second user message
        connection.set_next_prompt_updates(vec![acp::SessionUpdate::ToolCall(
        acp::ToolCall::new("tool2", "编辑文件 2")
            .kind(acp::ToolKind::Edit)
            .status(acp::ToolCallStatus::Completed)
            .content(vec![acp::ToolCallContent::Diff(
                acp::Diff::new("/project/test2.txt", "新内容 2").old_text("旧内容 2"),
            )]),
    )]);

        thread
            .update(cx, |thread, cx| thread.send_raw("再来一个", cx))
            .await
            .unwrap();
        cx.run_until_parked();
    ...
    async fn test_scroll_to_most_recent_user_prompt(cx: &mut TestAppContext) {
        ...
        connection.set_next_prompt_updates(vec![acp::SessionUpdate::AgentMessageChunk(
            acp::ContentChunk::new("响应 1".into()),
        )]);
        ...
        thread
            .update(cx, |thread, cx| thread.send_raw("提示词 1", cx))
            .await
            .unwrap();
        cx.run_until_parked();

        connection.set_next_prompt_updates(vec![acp::SessionUpdate::AgentMessageChunk(
            acp::ContentChunk::new("响应 2".into()),
        )]);

        thread
            .update(cx, |thread, cx| thread.send_raw("提示词 2", cx))
            .await
            .unwrap();
        cx.run_until_parked();
    ...
    async fn test_message_editing_cancel(cx: &mut TestAppContext) {
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("待编辑的原始消息", window, cx);
        });
        active_thread(&conversation_view, cx)
            .update_in(cx, |view, window, cx| view.send(window, cx));
        ...
        user_message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("已编辑的消息内容", window, cx);
        });
        ...
        user_message_editor.read_with(cx, |editor, cx| {
            assert_eq!(editor.text(cx), "待编辑的原始消息");
        });
    }
    ...
    async fn test_message_editing_regenerate(cx: &mut TestAppContext) {
        ...
        conversation_view.read_with(cx, |view, cx| {
            ...
            assert_eq!(new_editor.read(cx).text(cx), "已编辑的消息内容");
        })
    }
    ...
    async fn test_message_editing_while_generating(cx: &mut TestAppContext) {
        ...
        cx.update(|_, cx| {
            connection.send_update(
                session_id.clone(),
                acp::SessionUpdate::AgentMessageChunk(acp::ContentChunk::new("响应".into())),
                cx,
            );
    ...
    async fn test_interrupt(cx: &mut TestAppContext) {
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("消息 1", window, cx);
        });
        active_thread(&conversation_view, cx)
            .update_in(cx, |view, window, cx| view.send(window, cx));
        ...
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("消息 2", window, cx);
        });
        active_thread(&conversation_view, cx)
    ...
    async fn test_message_editing_insert_selections(cx: &mut TestAppContext) {
        ...
        user_message_editor.read_with(cx, |editor, cx| {
            ...
            let expected_text = String::from("带选择范围的待编辑原始消息");
    ...
    async fn test_insert_selections(cx: &mut TestAppContext) {
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("你能检查这段代码片段吗", window, cx)
        });
        ...
        message_editor.read_with(cx, |editor, cx| {
            ...
            let expected_txt = String::from("你能检查这段代码片段 选择内容");
    ...
    async fn test_tool_permission_buttons_terminal_with_pattern(cx: &mut TestAppContext) {
        init_test(cx);
            ...
            let tool_call_id = acp::ToolCallId::new("终端-1");
            let tool_call = acp::ToolCall::new(tool_call_id.clone(), "执行 `cargo build --release`")
                .kind(acp::ToolKind::Edit);
        ...
        conversation_view.read_with(cx, |conversation_view, cx| {
            ...
            assert!(tool_call.is_some(), "期望存在工具调用条目");
            ...
            assert!(
                ...
                "期望状态为 WaitingForConfirmation，实际为 {:?}",
                tool_call.status
            );
    ...
    async fn test_tool_permission_buttons_edit_file_with_path_pattern(cx: &mut TestAppContext) {
        ...
        conversation_view.read_with(cx, |conversation_view, cx| {
            ...
            assert!(
                labels.contains(&"始终允许 `src/` 目录"),
                "缺少路径匹配模式选项"
            );
            } else {
                panic!("期望状态为 WaitingForConfirmation");
            }
        });
    }
    ...
    async fn test_tool_permission_buttons_fetch_with_domain_pattern(cx: &mut TestAppContext) {
        ...
        let tool_call_id = acp::ToolCallId::new("获取-1");
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("获取文档", window, cx);
        });
        ...
        conversation_view.read_with(cx, |conversation_view, cx| {
                ...
                &tool_call.status
            {
                ...
                let labels: Vec<&str> = choices
                    ...
                    .collect();
                assert!(
                    labels.contains(&"始终允许获取操作"),
                    "缺少“始终允许获取操作”选项"
                );
                assert!(
                    labels.contains(&"始终允许 `docs.rs` 域名"),
                    "缺少域名匹配模式选项"
                );
            } else {
                panic!("期望状态为 WaitingForConfirmation");
    ...
    async fn test_tool_permission_buttons_without_pattern(cx: &mut TestAppContext) {
        ...
        let tool_call_id = acp::ToolCallId::new("终端-无模式-1");
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("运行部署脚本", window, cx);
        });
        ...
        conversation_view.read_with(cx, |conversation_view, cx| {
                ...
                &tool_call.status
            {
                ...
                // 不应该包含命令模式选项
                assert!(
                    !labels.iter().any(|label| label.contains("commands")),
                    "不应该存在命令模式选项"
                );
            } else {
                panic!("期望状态为 WaitingForConfirmation");
    ...
    async fn test_authorize_tool_call_action_with_pattern_option(cx: &mut TestAppContext) {
        ...
        let tool_call_id = acp::ToolCallId::new("模式操作测试-1");
        let tool_call =
            acp::ToolCall::new(tool_call_id.clone(), "执行 `npm install`").kind(acp::ToolKind::Edit);
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("安装依赖项", window, cx);
        });
        ...
        conversation_view.update_in(cx, |_, window, cx| {
            window.dispatch_action(
                crate::AuthorizeToolCall {
                tool_call_id: "模式操作测试-1".to_string(),
                option_id: pattern_option.option_id.0.to_string(),
                option_kind: "始终允许".to_string(),
            }
                .boxed_clone(),
                cx,
            );
    ...
    async fn test_granularity_selection_updates_state(cx: &mut TestAppContext) {
        ...
        let tool_call_id = acp::ToolCallId::new("粒度测试-1");
        let tool_call =
        ...
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("构建项目", window, cx);
        });
    ...
    async fn test_allow_button_uses_selected_granularity(cx: &mut TestAppContext) {
        ...
        let tool_call_id = acp::ToolCallId::new("允许粒度测试-1");
        let tool_call =
            acp::ToolCall::new(tool_call_id.clone(), "执行 `npm install`").kind(acp::ToolKind::Edit);
        ...
        thread_view.update_in(cx, |_, window, cx| {
            window.dispatch_action(
                crate::SelectPermissionGranularity {
                    tool_call_id: "允许粒度测试-1".to_string(),
                    index: 1,
                }
                .boxed_clone(),
                cx,
    ...
    async fn test_option_id_transformation_for_allow() {
        ...
        assert_eq!(
            allow_ids
                ...
                .count(),
            2,
            "期望存在两个 always_allow:terminal ID（一个完整工具，一个带子模式的模式）"
        );
    }
    ...
    async fn test_option_id_transformation_for_deny() {
        ...
        assert_eq!(
            deny_ids
                ...
                .count(),
            2,
            "期望存在两个 always_deny:terminal ID（一个完整工具，一个带子模式的模式）"
        );
    }
    ...
    async fn test_manually_editing_title_updates_acp_thread_title(cx: &mut TestAppContext) {
        ...
        cx.simulate_input("我的自定义标题");

        cx.run_until_parked();

        title_editor.read_with(cx, |editor, cx| {
            assert_eq!(editor.text(cx), "我的自定义标题");
        });
        thread.read_with(cx, |thread, _cx| {
            assert_eq!(thread.title(), Some("我的自定义标题".into()));
        });
    }
    ...
    async fn test_max_tokens_error_is_rendered(cx: &mut TestAppContext) {
        ...
        let message_editor = message_editor(&conversation_view, cx);
        message_editor.update_in(cx, |editor, window, cx| {
            editor.set_text("部分提示词", window, cx);
        });
        active_thread(&conversation_view, cx)
            .update_in(cx, |view, window, cx| view.send(window, cx));
        ...
        conversation_view.read_with(cx, |conversation_view, cx| {
            ...
            assert!(
                ...
                "期望为 ThreadError::MaxOutputTokens，实际得到：{:?}",
                error.is_some()
            );
        });
    }
    ...
    fn request_test_tool_authorization(
        ...
        let label = format!("工具 {tool_call_id}");
    ...
    async fn test_conversation_multiple_tool_calls_fifo_ordering(cx: &mut TestAppContext) {
        ...
        let session_id = acp::SessionId::new("会话-1");
        let (thread, conversation) = cx.update(|cx| {
            let thread =
                create_test_acp_thread(None, "会话-1", connection.clone(), project.clone(), cx);
            let conversation = cx.new(|cx| {
                let mut conversation = Conversation::default();
                conversation.register_thread(thread.clone(), cx);
                conversation
        ...
        let _task1 = request_test_tool_authorization(&thread, "测试调用-1", "允许-1", cx);
        let _task2 = request_test_tool_authorization(&thread, "测试调用-2", "允许-2", cx);
        ...
        cx.read(|cx| {
            let (_, tool_call_id, _) = conversation
                .read(cx)
                .pending_tool_call(&session_id, cx)
                .expect("期望测试调用-2在测试调用-1获得授权后处于待处理状态");
    ...
    async fn test_conversation_subagent_scoped_pending_tool_call(cx: &mut TestAppContext) {
        ...
        let parent_session_id = acp::SessionId::new("父会话");
        let subagent_session_id = acp::SessionId::new("子代理会话");
        let (parent_thread, subagent_thread, conversation) = cx.update(|cx| {
            let parent_thread =
                create_test_acp_thread(None, "父会话", connection.clone(), project.clone(), cx);
            let subagent_thread = create_test_acp_thread(
                Some(acp::SessionId::new("父会话")),
                "子代理会话",
                connection.clone(),
                project.clone(),
                cx,
        ...
        let _parent_task =
            request_test_tool_authorization(&parent_thread, "父会话工具调用", "允许父会话", cx);
        let _subagent_task =
            request_test_tool_authorization(&subagent_thread, "子代理工具调用", "允许子代理", cx);
    ...
    async fn test_conversation_parent_pending_tool_call_returns_first_across_threads(
        ...
        let session_id_a = acp::SessionId::new("会话-A");
        let session_id_b = acp::SessionId::new("会话-B");
        let (thread_a, thread_b, conversation) = cx.update(|cx| {
            let thread_a =
                create_test_acp_thread(None, "会话-A", connection.clone(), project.clone(), cx);
            let thread_b =
                create_test_acp_thread(None, "会话-B", connection.clone(), project.clone(), cx);
            let conversation = cx.new(|cx| {
                let mut conversation = Conversation::default();
        ...
        cx.read(|cx| {
            ...
            .expect("期望来自会话-B查询的待处理工具调用");
            assert_eq!(
                returned_session_id, session_id_a,
                "非子代理查询始终按先进先出顺序返回第一个待处理请求"
            );
            assert_eq!(tool_call_id, acp::ToolCallId::new("tc-a"));
        });
        ...
        cx.read(|cx| {
            let (returned_session_id, tool_call_id, _) = conversation
                .read(cx)
                .pending_tool_call(&session_id_b, cx)
                .expect("期望会话-A的工具调用获得授权后存在会话-B的工具调用");
    ...
    async fn test_move_queued_message_to_empty_main_editor(cx: &mut TestAppContext) {
        ...
        assert_eq!(queue_len, 0, "移动后队列应为空");
    ...
    async fn test_close_all_sessions_skips_when_unsupported(cx: &mut TestAppContext) {
        ...
        conversation_view.read_with(cx, |view, _cx| {
            let connected = view.as_connected().expect("应处于已连接状态");
            assert!(
                !connected.threads.is_empty(),
                "应至少存在一个会话线程"
            );
    ...
    async fn test_close_session_returns_error_when_unsupported(cx: &mut TestAppContext) {
        ...
        assert!(
            result.unwrap_err().to_string().contains("不支持"),
            "错误信息应明确提示不支持关闭操作"
        );
    }
